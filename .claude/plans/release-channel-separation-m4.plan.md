...
| `.github/workflows/version-declaration-gate.yml` | UPDATE | R2 — 주석만. `paths`는 유지(리터럴 재도입 탐지 입력) |
...
- **Action**: `scripts/release-manifest-guard.js`. `.claude-plugin/marketplace.json`을 읽어
  ...
  `source.path === 'plugins/mccp'` · **`sha` 키 부재**. `url` 존재도 함께 본다.
...
- **Action**: Task 5가 리터럴을 없애면 `htmlFooterVersion`/`markdownFooterVersion`이
  ...
  - 헤더 주석의 "무엇을 재는가" 열거를 함께 갱신한다. 열거와 코드가 어긋나면 다음 독자가
    코드가 아니라 열거를 믿는다.
- **약화되지 않는가**: 약화된다면 그것은 "footer가 아예 사라져도 가드가 조용하다"는
  형태일 텐데, 그 축은 가드가 아니라 `i18n-surface.test.js`가 렌더 **출력**에서 잡는다
  (footer 부재 → 정규식 미매치 → red). 두 검사의 분담을 가드 주석에 명시한다.
- **Validate**: 아래 검사 6.

### Task 8: 역방향 단언의 판별력 test (R2)

- **Action**: `scripts/tests/version-declaration-guard.test.js`에 두 케이스 추가 —
  (a) 파생 형태 소스 → `derived`, (b) 리터럴이 되돌아온 소스 →
  `version-face-literal-reintroduced`. (b)가 없으면 역방향 단언이 "항상 통과하는 검사"다.
...
# 4. 렌더러 두 면에 버전 리터럴이 0건
node -e "const fs=require('fs'); ['plugins/mccp/scripts/lib/renderer/html.js','plugins/mccp/scripts/lib/renderer/markdown.js'].forEach(function(f){var m=fs.readFileSync(f,'utf8').match(/v[0-9]+\.[0-9]+\.[0-9]+/g); if(m) throw new Error(f+' still carries version literal(s): '+m.join(','));}); console.log('ok: both renderer faces derive');"
...
node -e "const b=require('fs').readFileSync('.claude/prds/release-channel-separation.prd.md','utf8'); const rows=b.split('\n').filter(function(l){return /^\|\s*[0-9]+\s*\|/.test(l);}); if(rows.length!==4) throw new Error('milestone rows='+rows.length); const u=(b.match(/^- \[ \]/gm)||[]).length; if(u!==0) throw new Error('unchecked OQ='+u); console.log('ok: 4 rows, 0 unchecked');"
...
|---|---|---|
| Task 5(리터럴 제거)와 Task 7(가드 재배선)이 분리 착지하면 **모든 PR이 붉어진다** — 가드가 `version-face-unreadable`로 HALT한다 | 높음 | 둘을 **단일 커밋 불변식**으로 다룬다. 검사 4와 6을 같은 Validation 블록에 두어 한쪽만 착지한 상태가 green이 될 수 없게 한다. 가드 헤더 주석에도 그 결속을 적는다 |
