...
- Tests: `receipt/tests/block-format.test.js`(신규 8) · `hooks/tests/receipt-prompt-tamper.test.js`(신규 3) · `preflight.test.js`(+1 subject-tamper) · `lib/tests/history-leak-scan.test.js`(+3: F1 base-blob-new-path · F1 R2 ancestor-only-deleted-before-HEAD · F3 unresolved-base-fail-closed). Codex 수렴 loop 3라운드(#1 F1H+F2M → #2 F1 ancestor-only → #3 F3 unresolved-base), 매 라운드 실제 결함 정확히 좁힘. 버전은 1.22.6 유지(미머지 M2에 리뷰 흡수).
