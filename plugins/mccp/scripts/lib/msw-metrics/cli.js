...
function cmdA1(argv) {
  try {
    ...
    // 초과에서만 내면 그 조건은 오늘 도달하지 않아(실측 3 — santa R3 정정. 이 자리는
    // 23으로 적혀 있었고, 그 수치는 배너가 재는 값과 8배 가까이 어긋나 임계까지의
    // 거리를 실제보다 훨씬 가깝게 보이게 했다) 토큰이 다시 0개 표면이 되고, 이
    // Task는 표면을 열었다고 주장하면서 아무것도 열지 않은 것이 된다.
