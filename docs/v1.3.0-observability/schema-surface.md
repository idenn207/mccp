...
## §9 — Session-ledger surface (v1.5.0-m1)

**STATUS: implemented in v1.5.0-m1.** Per-session JSON ledger files written by SessionStart and finalized by SessionEnd. Canonical schema definition: [`docs/v1.4.0-multi-session/session-ledger-schema.md`](../v1.4.0-multi-session/session-ledger-schema.md). Public API: [`plugins/mccp/scripts/state/session-ledger.js`](../../plugins/mccp/scripts/state/session-ledger.js) (`resolveLedgerScope`, `createLedger`, `finalizeLedger`, `readLedger`, `listLedgers`).

**Crucial — STATE.md frontmatter is NOT modified by M1**: the first plan revision proposed adding `session_id` + `session_ledger_path` to §4 frontmatter; Codex Implement R1 F2 showed this is incompatible with the v0.3.6 hash-skip noise-elimination invariant (the `HASH_EXCLUDE_FRONTMATTER_KEYS` path in `state-writer.update` prevents anchor persistence when only excluded fields change). §4 of this doc remains the canonical STATE.md frontmatter schema; the session-ledger surface is an **additive new file class** under `<projectDir>/.session-ledgers/` (global default) or `<repo>/.claude/state/session-ledgers/` (repo opt-in). Discovery uses `listLedgers({activeOnly:true})` exclusively. See [`docs/v1.4.0-multi-session/state-md-narrowing.md`](../v1.4.0-multi-session/state-md-narrowing.md) for the role-narrowing explainer.

**derive consumer surface**: `derive/sources/state.js#scanState` now returns `item.active_session_ledgers: Array<{session_id, cwd, git_branch, created_at, host, pid, project_id}>` via the scope-aware `listLedgers` resolver (Codex Implement R1 F3 absorption — never hardcodes a repo path). Field names follow the ledger schema verbatim (`created_at`, NOT `started_at`; `git_branch`, NOT `branch`).

**`scanState` return contract — `item` is now non-null when STATE.md is absent**: prior to v1.5.0-m1 `scanState` returned `{item: null}` when `.claude/state/STATE.md` did not exist; v1.5.0-m1 returns a synthetic item (`frontmatter:{}`, `body:{}`, `resume_state:'idle'`, `controller_active:false`, `escalate_pending:false`, `path:null`, `active_session_ledgers:[…]`) so the ledger surface is reachable even on a fresh repo. Consumers that need to gate on STATE.md existence must check `item.path != null` (not `item == null`). v1.3.0-m3 verdict.js paths are not affected — every gated branch (`'giveup'` / `'in-flight'` / `escalate_pending` / `controller_active`) evaluates to `false` for the synthetic item.

**Privacy invariant (v1.5.0-m1 H1 absorption)**: `derive/mask.js#applyPathMask` walks `state.item.active_session_ledgers[*]` so `cwd` is repo-relative posix-normalized and `host` is redacted to the literal `<host>` placeholder. `git_branch`, `session_id`, `project_id`, `pid`, `created_at` stay raw (branches are public-ish, the rest are non-identifying). Both transforms live in the path-mask layer, so `--raw` (ops-debug only) bypasses them — the dashboard renderer (default-masked) never sees the operator's hostname or absolute cwd.

**Cross-schema isolation**: session-ledger is distinct from envelope (§3) and from observer-sessions lease files; the namespace subdir `.session-ledgers/` is **separate** from `.observer-sessions/` to avoid path collision (Codex Implement R1 F1 absorption). Helper layers (atomic tmp+rename, hand-rolled validate, `KNOWN_KEYS` Set, advisory `.lock` file with same constants as `state-writer.js`) are reused; the schema definitions are independent.
