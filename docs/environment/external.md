...
**허용 토큰** — 열거 없음. mccp가 정의하지 않는 외부 MCP 서버 이름이라 이 계약이 어휘를 소유하지 않는다 (UI6).

**미상 멤버** — 이 계약이 소유하지 않는 외부 MCP 서버 이름이라 어휘를 정의하지 않는다 (UI11). 미상 토큰의 처리는 ECC 쪽 소비처가 정하며 mccp는 이름을 읽어 넘길 뿐이다 (mcp-health-check.js:55)
