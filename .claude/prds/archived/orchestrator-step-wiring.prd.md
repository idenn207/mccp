...
| 4 | live-integrity-repair | 머지 뒤 라이브에서 재발한 결함이 닫힌다 — 테스트가 실 공유 corpus를 오염시키지 않고, 위치 의존성이 재발하면 사적 데이터를 가진 위치의 A1 배너가 그것을 말하며, `record-halt`와 `a1` 실패 경로가 절대경로를 흘리지 않는다. 이 PRD 축의 backlog·fix-task 잔여가 근거와 함께 처분된다 | complete | [`.claude/plans/orchestrator-step-wiring-m4.plan.md`](.claude/plans/orchestrator-step-wiring-m4.plan.md) · 결과 [report](.claude/PRPs/reports/orchestrator-step-wiring-m4-report.md) |
