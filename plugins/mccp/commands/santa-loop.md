...
- The delta kill switch is `MCCP_SANTA_DELTA_SCOPE` (`off` default / `enforce`), registered in docs/ENVIRONMENT.md §11 with the other santa toggles. **Its default is the opposite of every other santa toggle, deliberately.** The others default to firing because a typo that silently reverts a run to pre-axis behaviour is invisible; here firing is the *looser* direction — it removes files from review — and the cost of being wrong is the 16–93pp detection-rate drop the PRD cites. M2 measured the axis and **left the default at `off`**, by applying its own pre-registered rule rather than by judgement: the rule's antecedent is a *live-reviewer* comparison, M2 shipped only the deterministic containment layer, and an unmeasured antecedent is not a satisfied one. A regression test binds the shipped default to the recorded evidence, so flipping it without that evidence turns the suite red.
- What M2 did measure is **where the delta loses reach, not whether reviewers still find things**. On a synthetic corpus (N=1, one defect per class) run through the real CLI in both modes, the only containment loss is the class of defect living in a file the fix commit never touched — dropped wholesale, which is arithmetic, not a surprise. A defect in the *same* file but outside the context window keeps its path, so the "a range is a pointer, not a cut" design holds for that class; whether a reviewer's attention actually goes there is exactly the question the deterministic layer cannot answer, and it stays open. Details and limits: `.claude/notes/santa-delta-review-m2.md`.
