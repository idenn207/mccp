...
> 그 열은 삭제했고, 남은 외부 인용은 PRD 값 하나뿐이며 그것은 **PRD 의 것으로
> 명시**한다.
...
`git show <ref>:<path>` 다. 트리는 정의상 불변이므로 위 세 벡터가 한꺼번에 닫히고,
"ref 하나가 이 값들을 고정한다"가 산문이 아니라 사실이 된다. `--frozen-only` 가
방출하는 것:
