...
> **우산 PRD.** 자식 12개(C0~C11)의 색인이며, 자식은 각자 독립적으로 PRD·plan·ship·아카이브된다.
...
- **Secondary**: **marketplace로 mccp를 설치한 실사용자 — 그중 하나는 다른 프로젝트에서 mccp를 쓰는 운영자 자신이다** — **주당 약 13회**의 게이트 변경에 노출되고, 롤백 경로가 없으며(태그 1개), 그 변경 대부분이 자기와 무관한 dogfood 반복이다. 이 PRD가 이들에게 약속하는 것은 새 기능이 아니라 **노출 빈도의 감소와 롤백의 존재**다.
...
We'll know we're right when **오늘 산출되지 않는 5대 지표가 전부 실값을 갖고, 그 값이 읽히는 화면과 그 값이 바꾸는 행동이 각각 지정되어 있으며, 사용자 노출이 주당 약 13회에서 PRD 단위로 내려간다**.
...
| **C. 체감 (2차 사용자)** | 사용자 노출이 PRD 단위로 내려가고, 롤백이 manifest를 되돌리는 **수동** 절차로 성립한다 — 스키마에 자동 롤백은 없다(C0 `## References`) | 실사용자에게 이 PRD가 약속하는 유일한 것이다. 지표와 독립적으로 판정된다 |
...
| 2 | orchestrator-step-wiring | 무인 완주율이 읽는 위치에 좌우되지 않는다 — 집계 경계가 저장소 전체가 되고 분모가 단일 granularity를 가지며 값이 실제로 표시된다(M1). `/mccp:work`가 멈춘 step이 기록된다(M2). 기록 실패가 체인을 멈추지 않는다 | in-progress | [orchestrator-step-wiring.prd.md](orchestrator-step-wiring.prd.md) |
...
| 11 | closure-accounting | 부채 종결 계기가 `open: 0`을 보고하는데 그 분모(1115)가 라이브 부채(2487)의 **45%** 다 — 봉인은 2026-09-01에 멈췄고 재봉인·경보·CI 호출이 전부 부재하며(워크플로 6개 중 0개), 판정 1115건 중 `fixed`는 **1건**이다. 분모 격차·봉인 나이·두 종결 계기의 불일치가 한 출력에 선다 | in-progress | [closure-accounting.prd.md](closure-accounting.prd.md) |
...
| C11 | 1 | 없음 | 배포 | 낮음 (신규 read-only 산출물) | 불필요 |

**동시 착수 가능 최대치는 7개**다 — C0·C1·C2·C3·C4는 소유 파일이 완전히 분리되고, C6은 C0 착지만 기다리며, **C11이 신규 read-only 산출물만 소유해 어느 축과도 겹치지 않는다**(2026-09-08 추가). 근거 조사 본문에 5개와 6개가 함께 적힌 자기모순이 있었고 C6의 소유 파일이 어느 축과도 겹치지 않으므로 그때는 6개가 맞았다 — C11이 늘어 지금은 7개다.
