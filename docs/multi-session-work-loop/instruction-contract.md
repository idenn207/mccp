...
| S3.17 | 3.17 impeccable 탐지 계약 (v1.31.1 M1 · v1.31.2 M2 — impeccable-detection-contract) | on-demand | - | - | - | (b) 불성립 — 네 불변식이 모두 기계 판정을 갖는다. plugin/bare 네임스페이스 분리는 `impeccable-resolve.test.js`의 "bare invocation equals the literal name mccp command bodies call"이 단언하므로, M3가 재배선 없이 project-local 사본을 지우면 그 test가 red가 된다. 모호성 처리는 "ambiguous winner is reported as unknown, not guessed"가 `source`·`path`·`version` 셋의 null을 고정한다. **M2가 소비처 배선을 더했다**: 판정 권한이 `available` 하나라는 것은 `dep-check.test.js`의 `checkImpeccable` 채널 매트릭스 + 양쪽 로드 순서 순환 회귀와 `session-start-dep-check.test.js`의 배너 양방향이 고정하고, `.impeccable/` 극성은 `gitignore-provision.test.js`의 canonical 단언 + 양방향 drift lint가 고정한다(한쪽만 고치면 두 test가 동시에 red). 상세는 `docs/gate-design.md#impeccable-detection`이 소유한다(M2는 `#### setup·경고 정합 (M2)`). 이번 주기 미이전(분류만) |
