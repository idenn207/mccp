...
| B3 분모 | ~94 | 구현 시점 **94** (raw 104) → ship 시점 **101** (raw 111) · 명명된 제외 10 (rebase가 승계한 main 신규 토글 2개) |
| 은퇴 건수 | 0 | **0** |
...
| 5 | B3 분모 정직화 | 완료 | 제외 분류표 10건(file:line) · 분기 213(구현 시점 199 · ship 시점 재측정) · 이중 분모 |
...
> **분모는 ship까지 두 번 더 움직였다.** 구현 시점 104/94 → PR #118 rebase 후 106/96 → PR #120 병합 후 **111/101**. 매번 main이 새 토글을 들여왔고, 그때마다 재측정해 그대로 적었다. B3가 측정하려는 대상이 바로 이 증가이므로 분모를 고정해 수치를 지키지 않는다 — 제외는 여전히 10건이고 **은퇴는 0건**이다.
...
| G3 | B3 분모가 정직해지고 감축으로 위장되지 않음 | 제외 전/후 분모 병기(ship 시점 111 / 101 · 구현 시점 104 / 94) · 항목별 file:line · 분기 213 · **은퇴 0건** |
...
## santa-loop round 5b — (C3 세 번째 지적 수용)
