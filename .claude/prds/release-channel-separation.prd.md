...
| 4 | residual-closure | M1~M3이 명시로 이연한 부채를 닫는다. 좌표 파일 형태 단언이 사이클 단위에서 **모든 PR에서 도는 CI 게이트**가 되고(`release-manifest-gate`), 렌더러 두 footer가 번호를 `plugin.json`에서 **파생**해 컷이 움직이는 면이 다섯에서 셋으로 준다. 새 기능 0 · 원격 채널 ref 무이동 | complete | [.claude/plans/release-channel-separation-m4.plan.md](../plans/release-channel-separation-m4.plan.md) |
...
- [x] **롤백이 version 하향을 요구하는데 Claude Code가 그것을 수용하는가.** 문서는 "users only receive updates when it **changes**"라 적지 실패 조건을 말하지 않는다. 2.0.0에서 1.33.6으로 되돌릴 때 업데이트가 발생하는지 **M1의 라이브 검증이 답해야 한다.** 수용하지 않으면 롤백은 "version을 올리면서 내용을 되돌리는" 형태(예: `2.0.1` = 1.33.6의 코드)가 되고, 그건 결정 3의 번호 정책을 바꾼다. — **답(M1): 수용한다.** 리허설이 `release`를 되돌리자 CLI가 `Plugin "mccp" updated from 1.33.7 to 1.33.4`를 내고 설치 `gitCommitSha`가 그 좌표로 갱신됐다(force-push부터 update 완료까지 12초, 대체 경로 미사용). 따라서 결정 3의 번호 정책은 **바뀌지 않는다** — 롤백은 번호를 올리며 내용을 되돌리는 형태가 아니라 그냥 되돌리는 형태다. 근거 위치: [m1-report](../PRPs/reports/release-channel-separation-m1-report.md) Acceptance 2·4. **한정**: 측정된 하향은 patch 범위 1건이고 major 경계(`2.0.0 → 1.x`)는 미측정 — [docs/release-channel.md](../../docs/release-channel.md) 3.3절이 그 한정을 소유한다.
- [x] **버전 선언 위치를 `plugin.json`에서 marketplace entry로 옮길 것인가.** 문서상 marketplace entry의 `version`도 업데이트 신호가 될 수 있다. 옮기면 릴리스 컷이 한 파일만 고치고, `plugin.json`은 브랜치가 건드리지 않아 우산 결정 1이 **기계적으로** 강제된다. 옮기지 않으면 결정 1은 관례로만 남는다. 비용·부작용 미조사. — **답(M3): 옮기지 않는다. 질문의 전제가 거짓이 됐다.** "옮기지 않으면 관례로만 남는다"는 전제는 `scripts/version-declaration-guard.js`와 `.github/workflows/version-declaration-gate.yml`이 선언 위치를 **옮기지 않고** 기계화하면서 성립하지 않게 됐다 — 가드는 선언·반쪽 선언·CHANGELOG 번호 선점 세 축을 함께 재고 CI가 모든 PR에서 돌린다. 따라서 이것은 조사 대상이 아니라 기록 대상이고, 선언 위치는 `plugin.json`에 그대로 둔다. 그 결과 컷은 다섯 면을 한 커밋으로 움직인다([docs/release-channel.md](../../docs/release-channel.md) 2.3절). 렌더러 두 면이 번호를 manifest에서 파생하지 않는 부채는 별개 축으로 backlog가 소유하며, 그것이 닫히면 다섯 면이 세 면으로 준다.
