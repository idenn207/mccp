...
| 3 | STATUS.md + HTML renderer | derive model → `.claude/cache/STATUS.md` + `.claude/cache/status.html` 2 포맷. **6섹션**(worker fanout 추가) + cross-link badge + "Last refreshed" 상단 표시. envelope 미존재 시 fanout 섹션 graceful hide. | in-progress | [v1-3-0-observability-m3-renderer.plan.md](../plans/v1-3-0-observability-m3-renderer.plan.md) · [dashboard-surface](../../docs/v1.3.0-observability/dashboard-surface.md) |
