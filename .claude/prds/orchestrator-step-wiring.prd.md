...
|---|---|---|---|---|
| 1 | metric-boundary-unification | 어느 위치에서 derive를 돌려도 같은 A1이 나온다. 분모가 단일 granularity를 갖는다. 그 값이 `/mccp:work` 진입에 표시되고, 표시 라벨이 계산 단위(작업 단위)와 일치한다 | in-progress | `.claude/plans/orchestrator-step-wiring-m1.plan.md` |
| 2 | halt-step-recording | `/mccp:work`가 멈춘 step이 기록되어, A1이 하락했을 때 어느 phase가 막았는지 답해진다. 기록 실패는 체인을 멈추지 않는다 | pending | — |
