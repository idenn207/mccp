...
| 2 | arbiter 컨텍스트 분리 | 심판이 저자 컨텍스트에서 완전 분리(fresh subagent) — adjudication을 쓰는 주체가 plan 저자가 아니고, 저자 정당화에 도달할 경로가 코드에 없음 | complete | `.claude/plans/codex-intent-context-m2.plan.md` |
| 3 | cross-vendor 독립 2차 리뷰어(opt-in) | 중요 plan에 한해 이종 리뷰어 다양성 복원 — Codex(L3)와 이종인 Claude L2 패널이 **함께 발화한다**(배선 복구). 발화 대상의 기계적 판정은 범위에서 제외돼 `diverse-agent-review` PRD로 이관 | complete | `.claude/plans/codex-intent-context-m3.plan.md` |
...
> **M2 → M3 분리 근거 (2026-08-14)**: M2는 원래 "arbiter 분리 + cross-vendor 2차 리뷰어" 두 축을 묶고 있었다. 두 축을 함께 담은 plan 초안을 `/mccp:plan`의 L2 반박 패널(4관점)이 판정했고 blocking finding 9건으로 비수렴했는데, **CRITICAL 1 + HIGH 2가 전부 이종 리뷰어 축 하나에 몰렸다**. 심판 분리 축에서 나온 blocking은 1건뿐이었고 그 자리에서 흡수됐다. M1 → M1.5 분리와 같은 신호이므로 같은 처방을 적용한다 — 검증된 축(심판 분리)이 미검증 축에 묶여 못 나가는 것을 막는다. 두 축은 **단방향**이다: 심판 분리는 Codex가 발화하는 모든 경로에서 성립하고 이종 리뷰어 축에 의존하지 않는다.
>
> **M3의 설계 입력(패널 지적 3건, 다음 사이클이 그대로 받는다)**: (a) `hybrid`에서 runner를 어떤 시퀀스로 띄우는지 미명세 — 5.2z가 가진 detached 실행 + marker poll + lock-owner 분기를 hybrid용으로 확정해야 한다. (b) 이중 writer 방지가 test 단언 하나에만 걸려 있고 **runner가 5.6b보다 먼저 끝난다는 순서 보장**이 없다. (c) 승격 사실(`multi-agent` → `hybrid`)이 receipt에 봉인되지 않아 "왜 hybrid였나"를 사후에 읽을 수 없다. 선행 결함으로 이미 기록된 backlog 2026-08-12 HIGH(hybrid L3가 receipt-writing 경로에 위임)도 같은 축이다.

> **M2 `complete` 근거 (2026-08-21, v1.30.1)**: 심판 분리 축이 배송됐다. `/mccp:plan` 5.5a의 adjudication이 `Task(mccp:intent-arbiter)`로 이관되고, 그 에이전트의 `tools`가 `[Write]` 하나라 **plan을 여는 수단 자체가 없다**([intent-arbiter.md](../../plugins/mccp/agents/intent-arbiter.md)). 판정 입력은 `buildArbiterProjection`의 **whitelist**로 뽑혀 프롬프트에 인라인되며, whitelist는 최상위뿐 아니라 항목 안쪽에도 걸린다 — runner에 새 필드가 생겨도 자동으로 새어 들어오지 않는다. 강등은 원인을 열거하지 않고(에이전트 미등록·도구 거부·에러·취소·산출 부재·파손이 전부 같은 분기) create-exclusive 쓰기로 늦게 도착한 유효 산출을 덮지 않는다. 봉인 2필드(`intent_arbiter` · `intent_arbiter_degraded_reason`)는 present-only이되 **carve-out을 만들지 않아** hash 밖의 서명 없는 감사 필드가 생기지 않는다. Task 12건 전부 complete, e2e 9 시나리오가 `plan.md`의 `node -e` 프로그램을 추출해 실제 `write.js`까지 관통한다([report](../PRPs/reports/codex-intent-context-m2-report.md)).
>
> **이 `complete`가 뜻하지 않는 것**: (a) **심판이 옳아지지 않는다** — 저자의 근거를 볼 수 없게 될 뿐이다. (b) **위조 방지가 아니다** — 같은 권한으로 Node를 실행할 수 있는 주체는 receipt를 직접 봉인할 수 있고, 봉인값은 "subagent가 썼다"가 아니라 *요구된 모드와 관측된 강등*이다. (c) **기본 모드에서 intent 축은 여전히 skip된다** — `MCCP_PLAN_REVIEW` 미설정 → `multi-agent` → 패널 carve-out이며, 이 milestone의 게이트 실행이 그것을 실증했다. 귀속이 diverse-agent-review PRD라 M3 후보로 남긴다. 잔여의 안전 논증은 intent 축에 기대지 않는다 — 패널 승인은 cross-gate dedupe를 만족하지 못하므로 terminal `/mccp:pr`에서 PR-Codex가 반드시 발화한다. (d) **심판 판단의 품질은 이번 사이클에서 반증 불가다** — 배선은 test로 고정했고 품질은 머지 후 라이브 완주로 이연한다. (e) status는 이 브랜치의 편집이므로 **PR이 머지되는 시점에 참이 된다**.
>
> **게이트 receipt 복구 기록 (2026-08-21)**: plan/implement receipt는 working-tree only(§3.12)라 이 worktree가 갱신되며 소실됐다. 원본 게이트는 2026-08-15에 완주했고 그 증거는 git-tracked인 [review record](../reviews/plan-review-codex-intent-context-m2.md)(verdict `converged` via `multi-agent`, quorum 4/3, wall-clock 499741ms)와 [notes](../notes/codex-intent-context-m2.md)에 남아 있다. 복구 receipt의 `plan_hash`가 그 패널이 심의한 `sha256:9e22d72b…`와 **일치**하므로 plan 본문은 수렴 시점과 바이트 동일하다. 다만 transient `proof.json`이 소실됐고 `decide.js:6`이 `review_proof` hand-roll을 금지하므로 패널 증거는 receipt에 재봉인하지 못했다 — `MCCP_SKIP_INTENT_GATE` audited override로 기록했고 receipt는 실제 blocking verdict(`intent_gate_verdict=incomplete`)를 그대로 봉인한다. 따라서 cross-gate dedupe는 열리지 않는다.

> **M3 범위 확정 (2026-08-21)**: 사용자 판정으로 M3는 **배선 축만** 가져간다 — 죽은 `hybrid` opt-in 경로의 복구(전용 L3 호출로 `l3.json` 직접 산출 · 이중 writer 소거 · L3 사유 봉인)이며, 위 Outcome이 함께 적고 있는 **"발화 대상을 기계적으로 판정"(자동 트리거)은 이 milestone에서 만들지 않는다**. 그 축은 `diverse-agent-review.prd.md` #2 "L3 자동 트리거"가 A/B/C 신호 오라클로 이미 소유하고 있고, 그쪽은 #6 라이브 실측을 튜닝 입력으로 요구한다. M1 → M1.5, M2 → M3 분리와 같은 신호에 같은 처방이다: 검증된 축(배선)이 미검증 축(트리거 조건)에 묶여 못 나가는 것을 막는다.
>
> 근거가 된 실측 결함은 backlog `2026-08-12 HIGH`다 — `plan.md` 5.2f가 5.2z를 그대로 실행하라 지시하는데 5.2z는 receipt write를 소유한 `plan-codex-runner.js`를 띄우고 hybrid 경로에서는 `$CODEX_STDOUT`이 설정되지 않는다. 즉 opt-in 경로는 현재 **사용 불가**이며, 자동 트리거를 얹을 바닥 자체가 없다.

> **M3 `complete` 근거 (2026-08-21, v1.31.0)**: 배선 축이 배송됐다. `hybrid`는 오라클·스키마·receipt 필드가 M1에 전부 실렸는데도 **실행 경로가 죽어 있었다** — 5.2f가 "5.2z의 Codex 블록을 verbatim 실행하라"고 지시했고 그 블록이 띄우는 것은 receipt writer(`plan-codex-runner.js`)라, 패널 경로에서 5.6b와 writer가 둘이 됐다. M3는 L3를 receipt를 쓰지 않는 전용 서브커맨드 [`plan-review/cli.js l3`](../../plugins/mccp/scripts/lib/plan-review/cli.js)로 분리해 **순서 보장이 아니라 부재로** 닫았다(hybrid에서 runner가 존재하지 않으므로 순서 요건이 사라진다). 레코드 생산은 셸 `printf`에서 순수 오라클 [`buildL3Record`](../../plugins/mccp/scripts/lib/plan-review/l3.js)로 옮겨, fence를 넘은 빈 변수가 만들던 `"verdict":""`를 **구성 불가능**하게 했다. 승격 사실은 신규 필드 없이 `meta.review_l3_reason`(이미 `write.js`가 받는데 5.6b가 forward하지 않던 축)으로 봉인한다. PRD가 M3 설계 입력으로 넘긴 세 항목((a) runner 시퀀스 (b) 이중 writer 순서 (c) 승격 사실 봉인)이 각각 5.2f 재작성 · 소거 · `review_l3_reason` forward로 닫혔다. Task 10건 전부 complete, 회귀 292건 통과(1 skipped — Windows가 POSIX mode 비트를 재현하지 않아 0o600 단언만 건너뛴다). 상세는 [report](../PRPs/reports/codex-intent-context-m3-report.md) · [gate 산출물](../notes/codex-intent-context-m3.md).
>
> **이 `complete`가 뜻하지 않는 것 (정직 표기)**: (a) **발화 대상의 자동 판정은 없다** — 어떤 plan이 L3를 받을지는 여전히 사람이 env 두 개로 정한다. 위 Outcome이 원래 함께 적고 있던 그 축은 `diverse-agent-review.prd.md` #2 소관으로 이관됐다(UI2·UI3). (b) **Codex를 다른 벤더로 교체하지 않았고** 리뷰어 독립성은 완화까지만이다(UI7). (c) **라이브 완주는 부분 달성이다** — L3 층 자체는 실제 Codex 호출로 2회 완주해 `l3.json`에 `invoked:true` + enum verdict를 산출했으나(Acceptance 1 성립), receipt 축(Acceptance 2·3)은 `/mccp:plan` 전체 완주를 요구하고 그것은 하지 않았다. 이 plan에 대한 L2 패널이 실제로 `divergent`(quorum 4/3 미충족)라 전체 완주를 해도 converged hybrid receipt는 나오지 않으며, 재실행은 이 사이클이 검증에 사용한 receipt를 덮어쓴다. 대체 증거는 5.6b 추출 줄의 라이브 실행 + `write.js`를 실제로 관통하는 단위 test 2건이고, **초록 test를 완주로 부르지 않는다**. (d) **동시 실행 안전성은 얻지 못했다** — 라이브 L3-Codex가 낸 HIGH 2건 중 F2가 그것이고, `REVIEW_DIR` 전체가 공유하는 선재 성질이라 기각·backlog했다. 대신 nonce가 "동시 실행을 가른다"는 **과장된 주장을 3면에서 정정**했다(가르는 것은 stale뿐). (e) status는 이 브랜치의 편집이므로 **PR이 머지되는 시점에 참이 된다**.
