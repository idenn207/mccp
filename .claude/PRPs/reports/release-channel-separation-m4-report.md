...
추가 라인 기준 **0건**이다. 브랜치 전체(커밋 5개)로 넓히면 2건이 나오는데, 그 둘은 각각
plan(`42abeb7`)과 이 보고서(`30782f8`)에 적힌 **탐지기 자신의 정규식 리터럴**이다 —
`grep -cE 'C:\\+Users|/Users/|_project|…'`라는 **패턴 문자열**이지 경로가 아니다. 즉 검사
15는 자기 자신을 유출로 센다. 이것이 세 번째 결함이며 앞의 둘과 같은 축이다: 이 탐지기는
파일 전수를 보고, 삭제 파일을 다루지 못하며, 자기 패턴에 매치한다.

부수 관측 하나: 명령이 `.claude/state/fix-task.md`에
