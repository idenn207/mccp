...
**gate-guard-integrity M3 — 잔여 종료 (단일 milestone → patch bump, 1.25.1 → 1.25.2)** — M1·M2가 ship된 뒤 이 PRD가 저장소에 남긴 잔여물을 실측으로 확인하고 닫는다. 착수 시점 재확인(Task 0)에서 15행 중 **2행이 이미 해소돼 제거**됐고(아래 Observed), 나머지 13행을 처리했다.
