...
### 6a. 정정 (M2, 2026-09-03) — "플랫폼 차이"의 상당수는 플랫폼이 아니었다

위 절을 지우지 않고 남긴다 — 그 숫자들은 그때 실제로 관측된 것이고, 바뀜 것은
**그 관측을 무엇으로 읽느냐**다. M1은 원인 귀속을 하지 않았고(UI4·UI5) 그것은 옯았지만,
"어느 한 플랫폼에서만 돌리면 실패의 절반 이상을 구조적으로 못 본다"는 문장은 귀속 없이
이미 해석이었다. M2가 그 해석을 검증했고, 절반은 틀렸다.

**Windows 전용 실패 6건 중 3건**(`codex-invoke.test.js` · `codex-invoke-json.test.js` ·
`plan-review-cli-emit.test.js`)은 플랫폼과 무관하다. 이유는 둘이다:

1. 그 test들은 `codex-invoke` / `plan-review` 소비처가 **`gitDir`를 인자로 받는 시임**을
   갖고 있음에도 그것을 쓰지 않고 `process.cwd()` 폴백에 떨어졌다. 그래서 이 저장소의
   **살아있는 게이트 봉인**(`codex-policy.json` · `review-rounds-seal.json`)을 읽고
   `disabled` · `round-cap-reached`로 떨어졌다.
2. 이 저장소는 게이트 자체를 개발하므로 로컬에는 신선한 봉인이 상시 존재하고, CI는
   fresh checkout이라 그 봉인이 **없다**. 즉 갈린 것은 OS가 아니라 *저장소의 상태*였고,
   Windows는 단지 그 상태를 가진 쪽이었다.

반증은 간단했다 — 그 파일들을 러너 없이 `node --test`로 직접 돌려도 같이 실패한다.
따라서 M2 계획이 쓴 "`run.js`가 저장소 루트를 cwd로 자식을 띄우므로"라는 귀속도 틀렸다.
러너는 이 갈래에 관여하지 않는다.

**그래서 matrix 필요 여부는 다시 물어야 한다.** 위 표의 `win ∩ linux = 2`는 ambient 봉인
오염을 포함한 수치다. 그 3건을 걷어내면 Windows 전용은 3건이 되고, 그중 2건(갈래 R —
`validate-cmd.test.js` · `review-single-pass-fields.test.js`)은 병렬 하 git spawn 실패라
**플랫폼이 아니라 부하**일 가능성이 남아 있다. **M2는 이 질문을 다시 열기만 하고
답하지 않는다** — OQ3는 M3 소유다(UI6).

이 정정이 무효화하지 **않는** 것: Linux 전용 실패 7건은 여전히 Linux 전용이고,
그것만으로도 "한 플랫폼만 돌리면 못 보는 실패가 있다"는 명제는 살아있다. 약해진 것은
그 명제가 아니라 **그 명제가 기대는 수치**다.
...
- **`local` 측정 자체는 오염되지 않았다 (검증됨).** 그 실행 창은 산출의 `first_at`/`last_at`으로
  **11:43:33 → 12:14:56**이고, 뒤에 기술한 중단된 재측정이 남긴 orphan node 프로세스 289개는
  전부 **13:18 이후** 시작이다(실측). 즉 §2의 64.8배는 경합 없던 창에서 뜬 값이다. 그 orphan
  대부분(146 + 143)은 codex plugin의 `app-server-broker.mjs`/`codex.js`로, 스위트의 codex 관련
  test가 띄운 자식이 중단 시 회수되지 않은 것이다 — **러너의 결함이 아니라 그 test들의 정리
  경로 문제**이며 별도 축으로 남긴다.
