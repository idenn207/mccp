...
이 저장소는 게이트 기계를 만드는 데는 능하고, **그 기계를 부르는 한 줄을 빠뜨리며, 그 부재를 볼 수 있는 test가 없다.** 세 증상이 같은 뿌리를 갖는다 — 지표 producer가 구현됐는데 caller가 0회이고, 감사 corpus의 `rounds` 필드가 71/72에서 실제 라운드를 담지 않으며, merge-apply escape가 파일명 한 글자 차이로 조용히 미실행된다. 성공 방향 기본값(`converged: true, rounds: 1`)이 배선 단절을 **무증상**으로 만들고, CI가 368개 test 중 10개(2.7%)만 실행해 그 무증상을 확정한다.
...
- **CI 강제 커버리지 2.7%** (10 / 368 — C3 재측정으로 정정). 이 자리에는 `3 / 346`이 적혀 있었는데 세 번째 workflow `env-contract-drift.yml`(glob으로 7개 실행)을 세지 않은 값이었다. 방향은 불변이다. 그리고 세 workflow 전부 `paths:` 필터를 가지므로 **2.7%는 상한이고 PR당 실효 실행률은 더 낮다**. test는 있고 green이다 — 전수 실행 로그는 **353행 / 352 PASS / 1 FAIL**(`derive/tests/mccp-fixture.test.js`)이며, 그 1건은 C3 worktree 재실행에서 **green으로 관측**됐다(재현 미확인 → red가 아니라 flaky 후보다). 저장소는 red가 아니라 **아무도 안 돌린다**가 정확하다.
...
| **B. 방향 (2차)** | 지표 3·5는 100%, 지표 4는 20% 미만. 지표 1·2의 목표치는 **축 A 달성 후 baseline 2주로 확정** | 100%와 20%는 오늘 값(0% · 2.7% · 50%)이 있어 방어 가능하다. 1·2는 오늘 값이 없어 지금 정하면 근거 없는 숫자가 된다 |
...
| 5 | CI 강제 커버리지 (CI가 실행하는 test / 전체) | **2.7%** (10/368) | 산출된다 | **100%** | PR 체크 → 미달 시 머지 차단 |
...
| 3 | ci-full-suite | CI가 전체 test를 강제한다. 그 전에 실행 시간이 신뢰 가능해진다 — 오늘 타이밍은 경합 오염으로 자체 무효 판정됐다. **격차는 더 이상 미설명이 아니다**(자식 Evidence: `mkTmpRepo`가 repo당 git 프로세스 6개를 spawn하고 최상위 파일이 그것을 52회 부른다 — 48개 파일이 같은 helper를 쓴다). red 1건은 **flaky 후보로 재분류**됐다 — 재실행에서 green | in-progress | [ci-full-suite.prd.md](ci-full-suite.prd.md) |
