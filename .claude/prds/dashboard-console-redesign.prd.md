...
| 2 | 섹션 콘텐츠 + derive 실데이터 추출 | hero 판정·4축 legend·미해결질문·위험·게이트 파이프라인 스테퍼·타임라인·마일스톤 기록을 샘플 마크업으로 렌더하고, 각 더미 자리를 derive 엔진이 `.claude/` source에서 추출한 실데이터로 채움. 추출에 필요한 read-side surface(+ 최소 스키마 확장) 추가. | in-progress | `.claude/plans/dashboard-console-redesign-m2.plan.md` |
