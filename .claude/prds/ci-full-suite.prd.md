...
| 4 | enforcement-live-closure | M3이 만든 게이트에 **차단력을 붙이고** 그 차단력을 실증한다. 오늘 `main`은 `protected:false`이고 PR #185 자신이 green 게이트를 지나 머지됐다 — 게이트는 돌지만 막지 않는다. 넷을 닫는다: 축 D 절단 A·B의 CI red 실증(run 2건) · OQ3의 baseline dispatch 측정(artifact 4건) · 그 측정으로 격리 6건 재판정(Task 0 회수) · branch protection 설정과 `mergeStateStatus:BLOCKED` 관측. **새 기계를 만들지 않고 이미 만든 기계를 부른다** — 코드 변경은 선행 결함 둘뿐이다(절단 B 대상 가드 부재 · `ci-required-checks.js`가 admin 전용 채널을 읽어 non-admin에게 설정과 무관하게 `protection_absent`를 보고). **알려진 제약**: 인증 계정 `madsci207`은 `admin:false`라 protection 설정은 `idenn207`이 수행해야 하며, 권한이 오지 않으면 축 C를 **명시 미충족으로 두고 ship한다**(반올림 금지). 사용자 판단 6건은 AskUserQuestion 대신 **fable × codex 2중 리뷰**로 대체했고 판정표가 plan에 있다. **결과(2026-09-08): 넷 중 하나 충족 · 셋 미충족 — 반올림하지 않는다.** 충족: **OQ3 baseline dispatch**(run `34195014409` · 4 leg success · DD8 행 `matrix` 재도출 · "결정됨 · 시행 미완")와 그 측정에 의한 **격리 6건 재판정**(전건 잔존 · 사유 갱신). 미충족: **축 D**(원격 왕복 미수행 — 선행 코드 결함은 닫혔다)와 **축 C**(`madsci207`의 `admin:false`, 수행 주체는 `idenn207`). 코드는 계획대로 선행 결함 둘만 바꿨다 — 절단 B 대상 가드(5 사유 코드 · mutating call 앞 거부)와 판독 채널 world-readable 교체(+ 포함 검사 · 빈 `declared` fail-closed · `gh` 스텁 spawn seam). 로컬 게이트 exit 0 · 4파일 109 test pass. ~~**status가 `complete`가 아닌 것은 의도다** — 라이브 산출물 다섯 중 넷이 미충족인 상태를 `complete`로 적는 것이 이 PRD가 금지한 반올림이다.~~ **정정(2026-09-11 · 축 D 완주)**: 운영자 승인으로 축 D 원격 왕복을 수행했다 — 버리는 PR #193 에서 절단 A run `34556517175`(`stage:1`·`suite_red`) · 절단 B run `34566171941`(`stage:2`·`deleted_without_allowance`), `gate.json` tracked 사본 2건. 이제 미충족은 축 C 하나뿐이고 그 미충족은 Acceptance 7 이 정규 종착으로 규정한 **명시 미충족**(`axis-c: unmet` + 사유)이므로 status 를 `complete` 로 올리는 것은 반올림이 아니라 plan 이 설계한 종착역이다. 상세: [m4-live-closure.md](../../docs/ci-full-suite/m4-live-closure.md) | complete | `.claude/plans/ci-full-suite-m4.plan.md` |
| 5 | findings-closure | ~~findings registry가 흡수 경로의 close를 기록해 open 승입(opened 1678 / closed 19 — 2026-09-11 실측)이 수렴하고~~ open CRITICAL 8건(redact 6 · enumerate 1 · run 1)이 file:line 근거와 함께 c3 HEAD에서 재검증되고, 닫힘이 입증된 건이 **판정 원장**(`debt-inventory.js dispose`)용 batch로 준비된다. **정정(2026-09-14 · c3 재측정)**: registry에 close를 쓰는 원안은 이미 닫힌 경로다 — `feedback-loop-design.md` §2·§4 · `debt-inventory.js` 헤더(C1 분자 조작) · closure-accounting M3의 합성 producer 금지(DAR #1.5 이관). 승입 억제는 판정 원장을 읽는다(`handoff-items.js`). ~~8건은 봉인(2026-09-01) 이후에 열려 지금 `dispose`가 거부하므로~~ **재정정(2026-09-14 · 병합 후 실측)**: closure-accounting M2(PR #194)가 main에 머지되며 라이브 재봉인(`sealed_at 2026-09-08T08:25:23.521Z` · 2841건)이 8건을 **전부** 포함해 V3는 이제 `ready`다 — 봉인은 더 이상 장애가 아니다. 남은 선행 의존은 M2 행 자신이고(main에서 `in-progress`) Task 3 트리거가 그 행의 `complete`를 요구하므로 **적용은 여전히 보류**다(운영자 선택 "main 병합 · 적용 보류"). **산출(Task 0~2)**: 판정은 `fixed` 2(m3a `639d7374f2f4d904` · m3b `a1ce669aebc9a95a` — 둘 다 PR #185가 닫았고 `coverage.js:98`·`gate.js:189`가 근거) · `open` 6(m1 santa — claim 원문이 `.gitignore:53`으로 무시되는 santa 원장에 있었고 부재해 개별 입증 불가). batch 2줄은 `docs/ci-full-suite/m5-dispositions.jsonl`에 준비됐고 ~~적용 0건~~, 근거 문서는 `docs/ci-full-suite/m5-findings-closure.md`다. ~~그 전까지 이 행은 `in-progress`를 넘지 않는다.~~ **정정(2026-09-15 · Task 3 적용 · 종결)**: 운영자 승인으로 보류를 해제하고 `dispose`를 적용했다 — `appended 2` · `rejected 0` · verify `fixed` 1→3 · `open` 1740→1738 · `invalid` 0 · `binding_mismatch` 0, 두 건이 `suppressedFindingIds`에 들어가 SessionStart 승입 표면에서 부재. **트리거 문구는 문자 그대로 미충족인 채 적용했다** — closure-accounting M2 행은 main과 c11 커밋 모두 `in-progress`(`complete`는 c11 미커밋 편집에만 있음). 트리거가 지키려던 실질 전제는 적용 직전 전부 재확인했다: V3 `ready` · M2 라이브 재봉인 `after.denominator_gap.count 0` · c11 후속 변경이 batch 계약 무변경 · V1~V5 통과. `open` 6건은 닫히지 않았고 이 종결은 그것을 주장하지 않는다 — M5의 산출(8건 재검증 · 입증분 판정 원장 적용 · c2 잔재 되돌림)이 완결됐다는 뜻이다. 상세: `m5-findings-closure.md` §4.1 **출처**: `c2-orchestrator-step-wiring` worktree에서 M4 slug로 잘못 작성된 plan의 고유 잔여 범위를 분리했다(2026-09-14) — 그 plan의 Task 3·4·5는 이 PRD의 M4가 이미 회수했다. **인수 잔재**: 그 worktree의 09-11 세션이 close 이벤트 77건(`fixed` 8 · `deferred` 69)을 근거 없이(`cited_path:null`, 작성 스크립트 부재) 16개 shard에 미커밋으로 남겼다 — 위 경로 판정에 따라 정합화가 아니라 **되돌림**이다(Task 2). **완료(2026-09-14)**: 77줄 전부 제거 · marker 삭제 · 비대상 이벤트 0건 · V4 통과. 방법은 `git checkout`이 아니라 **식별된 줄만 제거**다(Implement-Codex R1 F1 흡수 — c2에 live 세션이 있고 registry가 잠금 없이 append하므로 파일 전체 되돌림은 그 사이 들어온 정당한 이벤트를 지운다) | complete | `.claude/plans/ci-full-suite-m5.plan.md` |
