...
| 3 | patch-chasing terminator + 캡 정책 | 직전 수정만 겨누는 라운드에서 루프가 스스로 종료하고, 종료 사유가 기록됨 | complete | [santa-adjudication-m3.plan.md](../plans/santa-adjudication-m3.plan.md) |
...
- [ ] **`issue_id`를 `locations`에서 파생할지** — M2 Open Question의 처방 (1)("리뷰어에게 안정적 식별자를 요구하고 `issue_id`를 claim이 아니라 거기서 파생할지")이 M3 또는 P2의 축으로 남겼고, **M3은 그 축을 취하지 않았다**(DD3 말미). 파생을 바꾸면 기존 원장의 모든 id가 갈려 M2가 배송한 suppression이 그 시점에 전멸하고, `locations`가 선택 필드인 이상 절반의 지적은 id를 얻지 못한다. M3이 그 축을 **가능하게** 만든 것은 사실이므로(리뷰어가 실제로 `locations`를 채우는지가 이제 `targetsBreakdown`으로 관측된다), 판단은 실측값을 본 뒤에 한다 — 판정의 입력은 아래 M3 실경로 관측의 `unknown` 비율이다.
  - **실측(2026-08-18, M3 Task 8): 대조가 가능했던 전 라운드에서 `unknown`은 0이다 — 라운드 1~3의 blocking 13건 중 0건.** 리뷰된 라운드는 4개(index 0~3, blocking 합 17)이고 `.claude/notes/santa-adjudication-m3.md`의 (B) 표가 원자료다. **라운드 0의 `unknown` 4건은 리뷰어 미준수가 아니다** — 라운드 0에는 직전 패치가 정의상 없어 `--prev-fix-rev`가 넘어가지 않고 `patchRanges`가 빈 집합이라 전량이 구조적으로 `unknown`이 된다(그 자리의 미발화 사유는 `round-below-min`이다). 따라서 이 축이 재는 값의 분모는 13이지 17이 아니다. 두 리뷰어(opus · gpt-5.4)는 `locations`를 **전 blocking에 빠짐없이** 채웠고 `contract`는 리뷰된 4라운드 전부 `full`이었다. 즉 "리뷰어가 선택 필드를 안 채운다"는 우려는 이 표본에서 재현되지 않았고, 라운드 ≥ 1의 미발화 원인은 언제나 `preexisting`(대조는 됐고 patch 밖)이었다. (증거 파일 `.claude/reviews/santa-review-santa-adjudication-m3-probe.md`가 `rounds: 4`로 봉인된 것은 seal이 **종료 시점**을 찍기 때문이고, `MCCP_SANTA_TERMINATOR=off` 재개가 연 index 4는 리뷰 없이 열리기만 한 라운드다 — 그래서 "5라운드"로 세지 않는다.) **그럼에도 `issue_id`를 `locations`에서 파생하는 축은 여전히 취하지 않는다** — 표본이 두 모델 · 한 파일 · 4라운드로 좁고, 위에 적은 "id가 갈리면 M2 suppression이 전멸한다"는 대가는 표본 크기와 무관하게 그대로다. 이 실측이 바꾼 것은 *가능성*의 평가이지 *비용*이 아니다.

- [ ] **M3 실경로 관측 — 전량 조건이 실제로 얼마나 자주 성립하는가(신규, M3 Task 8이 연 축)** — probe의 리뷰된 4라운드(index 0~3) `targetsBreakdown`은 라운드 0이 대조 불가(전량 `unknown`, 사유 `round-below-min`)이고 라운드 1이 6 `round_n_patch` / 1 `preexisting`, 라운드 2가 4 / 1, 라운드 3이 1 / 0이었다. **앞의 두 라운드는 단 한 건의 `preexisting` 때문에 미발화했고**, 그 한 건은 두 번 다 "리뷰어가 결함의 소재를 함수 본문이 아니라 시그니처 줄이나 미변경 줄로 지목한 것"이었다(각각 `src/path-guard.js:5`의 symlink 지적, `:21`의 root-dir 엣지케이스). 전량 조건은 설계대로 보수적으로 작동했다 — 오발화는 0건이다. 열린 질문은 그 보수성의 대가다: 직전 패치가 파일 일부만 고치는 한 리뷰어 한 명의 coarse한 위치 표기가 종료를 무기한 미룰 수 있고, 실제로 종료는 **직전 라운드가 모듈 전체를 재작성한 뒤에야** 성립했다. 처방 후보 둘 — (a) 전량이 아니라 비율 임계(UI19가 금지하는 방향이므로 채택하지 않는다) · (b) Step 3 프롬프트가 "결함을 고치려면 바꿔야 할 정확한 줄"을 요구하도록 재작성(M3이 라운드 2·3에서 실제로 시도했고 라운드 3에서 `preexisting` 0을 얻었다 — 인과는 미확정, 표본 1). **(b)의 재측정이 이 항목의 다음 작업이다.**
...
- [ ] **M2 Acceptance (B) 미충족 — 재보고 억제가 실경로에서 관측되지 않았다. 원인은 표본이 아니라 패러프레이즈이고, 실측으로 확인됐다.** M2 Task 7의 종자 결함 probe를 2026-08-17에 완주했다(별도 워크트리 `santa-m2-probe`, DD13 라운드 결속 `entry.round < N` → `<= N` 되돌림, decision slug `santa-adjudication-m2-probe`, 리뷰어 4명 전원 opus). 앞 두 축은 관측됐다:
  ...
  **(g)가 실패한 이유는 리뷰어가 결함을 못 찾아서가 아니다.** round 0과 round 1의 fresh 리뷰어 4명은
  같은 결함을 **4/4 전건** 다시 찾았고, 4건 모두 **다른 문장으로** 썼다 — 예: `"Self-suppression
  vulnerability: judgment in round N can now suppress findings in round N"`(round 0 A) vs
  `"Self-suppression vulnerability: same-round judgment can now suppress same-round blocking issue"`
  (round 1 A). `issue_id`가 정규화 claim이므로 넷 다 새 id를 얻었고 억제는 0건이었다. **DD5가
  High로 예측한 패러프레이즈 한계가 실측으로 확인된 것이고, 재보고율이 아니라 재보고의
  문안 안정성이 병목이다.**

  `carryOver`는 DD5가 정의한 서명을 정확히 냈다 — round 1에서 `{suppressed:0, resolvedAbsent:4,
  newBlocking:4}`. 즉 계측 도구는 의도대로 작동하며 이 사건을 **관측 가능하게** 만들었다.
  억제 메커니즘 자체는 고장이 아니다: fixture repo를 지나는 커버리지 34·41~46·49·55와 실 CLI
  왕복 스모크가 정확 재보고 경로를 덮고, probe에서도 (종자가 라운드 결속을 지웠으므로) round 1의
  판정이 **같은 라운드**의 그 지적을 실제로 지웠다(`kind: absorbed-rereported`, `entryRound: 1`,
  blocking 4 → 3) — 즉 우회가 실경로에서 재현됐고, 본 브랜치에서는 같은 시퀀스가 NAUGHTY로 남는다.

  **처방(미결)**: (1) 리뷰어 프롬프트가 지적마다 안정적 식별자(파일:라인 또는 규칙 id)를 함께
