...
계획 Acceptance의 라이브 완주 산출물 넷 중 **넷 다 완충족은 아니다.** 초판에서는 넷 다
미충족이었고 사유가 하나였다 — 브랜치가 원격에 없어 CI가 한 번도 돌지 않았다. 그 사유는
2026-09-08 소멸했다(§7a): 게이트가 발화했고 차단했다. 그래서 1·2는 **부분**으로 올라가고
3·4는 그대로 남는다. "발화했다"는 "green이다"가 아니므로 반올림하지 않는다.

| # | 요구 | 상태 | 무엇이 남았나 |
|---|---|---|---|
| 1 | PR에서 `test-suite` 체크가 발화하고 green | **부분** | 발화는 실증됐다(§7a) — green은 미실증 |
| 2 | 커버리지 실값이 **CI에서** 산출 | **부분** | CI producer(`gate.json`)의 존재는 실증됐다(§7a). 그 산출물은 stage 1에서 멈춰 `coverage: null`이라 실값은 아직 로컬 것뿐이다(§3) |
...
남는 것: green run은 아직 없다. 이 절은 "발화했고 옳게 막았다"까지만 주장한다.
