...
| 1 | Derive engine | `.claude/` 스캔이 plan / receipt / STATE / backlog / fix-task / PR / **dispatch envelope** 7 소스를 단일 정규화 model로 통합. mccp 외 임의 repo에서도 graceful fallback (envelope 미존재 OK). | complete | [v1-3-0-observability-m1-derive-engine.plan.md](../PRPs/plans/completed/v1-3-0-observability-m1-derive-engine.plan.md) |
| 2 | LLM briefing stamp + cost telemetry | `receipt-write` hook에서 1회 LLM 호출 → `meta.briefing_summary` 자동 stamp. **호출 telemetry(token count + invocation count)** 같이 stamp. cost-tier $50 notice 도달 시 briefing 자동 disable + raw-only fallback. | complete | [v1-3-0-observability-m2-briefing-stamp.plan.md](../PRPs/plans/completed/v1-3-0-observability-m2-briefing-stamp.plan.md) · [report](../PRPs/reports/v1-3-0-observability-m2-briefing-stamp-report.md) |
| 3 | STATUS.md + HTML renderer | derive model → `.claude/cache/STATUS.md` + `.claude/cache/status.html` 2 포맷. **6섹션**(worker fanout 추가) + cross-link badge + "Last refreshed" 상단 표시. envelope 미존재 시 fanout 섹션 graceful hide. | complete | [v1-3-0-observability-m3-renderer.plan.md](../PRPs/plans/completed/v1-3-0-observability-m3-renderer.plan.md) · [report](../PRPs/reports/v1-3-0-observability-m3-renderer-report.md) · [dashboard-surface](../../docs/v1.3.0-observability/dashboard-surface.md) |
| 4 | Refresh · failure · privacy guard | SessionStart + receipt-write + **envelope write/move** trigger. loud fail-open. secret / 경로 마스킹(envelope payload 포함). 60초 초과 시 노란 경고. | complete | [v1-3-0-observability-m4-refresh-failure-privacy-guard.plan.md](../PRPs/plans/completed/v1-3-0-observability-m4-refresh-failure-privacy-guard.plan.md) · [report](../PRPs/reports/v1-3-0-observability-m4-refresh-failure-privacy-guard-report.md) |
| 5 | Daily snapshot + decision log | `.claude/cache/snapshots/YYYY-MM-DD.json` 30일 archive + audit timeline derive(최근 7일). snapshot에 envelope 상태 freeze 포함. | complete | [v1-3-0-observability-m5-snapshot-decision-log.plan.md](../PRPs/plans/completed/v1-3-0-observability-m5-snapshot-decision-log.plan.md) · [report](../PRPs/reports/v1-3-0-observability-m5-snapshot-decision-log-report.md) |
| 6 | Generic interface 검증 | mccp 외 임의 repo `.claude/`에서 graceful fallback 동작 검증(envelope 없는 repo 포함) + reference impl로서 mccp-repo dogfood. | complete | [v1-3-0-observability-m6-generic-interface.plan.md](../PRPs/plans/completed/v1-3-0-observability-m6-generic-interface.plan.md) · [report](../PRPs/reports/v1-3-0-observability-m6-generic-interface-report.md) |
...
*Status: COMPLETE — all milestones (M0~M6) shipped as PRs #31/#33/#34/#37/#39/#41/#44 between 2026-06-17 and 2026-06-19. Cycle closed.*
