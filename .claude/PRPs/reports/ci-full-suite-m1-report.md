...
전수 test 실행의 정본 진입점을 배포 표면 밖(`scripts/`)에 만들고 로컬 1회 + CI 2회(Node 20·24)
...
| 벽시계 baseline | 로컬 1건 | 로컬 1 + CI 2 (matrix가 무료라 둘 다 실었다) |
...
| 6 | 측정 수행 (6갈래) | 완료 | 로컬 1 + CI 2 + 병합 + 분해 + flaky 3회 + 하한 |
| 7 | `docs/ci-full-suite/m1-baseline.md` | 완료 | 11절 |
...
| Integration | Pass | 좁은 범위 smoke 12파일 `ok:true`/`complete`/exit 0 · 전수 3회 완주 |
...
| 3 | `pull_request` run ≥1 + artifact 내용이 1번 검사 통과 | **MET** — run `33597753311`, 두 artifact 모두 통과 |
| 4 | 판정 문서에 상위 15 + flaky 3회 | **MET** |
| 5 | 자기 포함 (`scripts/tests/test-suite.test.js` 열거됨) | **MET** — 비-`.test.js` 3개는 미열거(정상) |
| 6 | 컨테이너 절대경로 0건 | **MET** — 3원소 전부 `redaction_ok:true`, `redaction_hits:[]` |
