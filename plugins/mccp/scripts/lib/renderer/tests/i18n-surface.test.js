...
test('html — footer version v1.20.1 (footer element anchored)', () => {
  const r = renderWithStubs(makeFullModel(Date.now()));
  ...
  assert.match(r.html, /<footer[^>]*>v1\.20\.1 ·/);
...
test('markdown — footer with v1.20.1 version', () => {
  const r = renderWithStubs(makeFullModel(Date.now()));
  assert.match(r.md, /v1\.20\.1/);
});
