...
| 3 | degrade 차단 | 동일모델 앙상블의 NICE가 이종 조합의 NICE와 구분되어 사람 승인을 요구함 | complete | [santa-evidence-diversity-m3.plan.md](../plans/santa-evidence-diversity-m3.plan.md) |
...
- [x] **`degraded` 판정의 하류 취급** — **해소(M3, 2026-08-19).** 세 가지로 확정했다. (a) **막는 것은 push다** — `santa-loop.md` Step 5.5가 `degraded`를 `converged`보다 **먼저** 검사해 exit 1로 push를 막는다(`!= converged` 절이 앞서면 degraded가 divergent 메시지에 흡수되어 처방이 사라진다). santa receipt가 게이트를 통과시키지 않는다는 I4는 그대로다 — degraded가 막는 것은 다른 mccp 게이트가 아니라 이 루프 자신의 push다. (b) **승인은 audited env다** — `MCCP_SANTA_DEGRADE_ACK="<substantive reason>"`(strict `validateReason`)가 있으면 push가 진행되고, ack와 그 사유는 receipt에 양방향 결속으로 봉인된다(한쪽만 있는 receipt는 schema가 거부한다). (c) **verdict는 재작성되지 않는다** — ack는 push를 열 뿐이고 봉인은 `degraded` 그대로다. codex 미설치 머신에서는 모든 실행이 degraded라 ack가 상주 설정이 되는데, ack가 verdict를 바꾸면 degraded 실행 수가 영구히 0이 되어 지표 4가 측정 대상을 잃는다. 어휘 경계에서는 `divergent`로 **좁혀 사영**하고(receipt/proof) `degraded`라는 이름은 santa 자신의 표면에만 둔다 — 공유 `REVIEW_VERDICT_VALUES`에 값을 더하면 폭발 반경이 receipt 계층 전체가 되기 때문이다.
- [ ] **블라인드 레인의 토큰 비용** — 저장소 재탐색은 번들 소비보다 비싸다. 실측 후 스코프 힌트 수준을 조정.
- [ ] **`off` 모드의 UI3 미충족을 무엇이 막는가** — M1 계획의 DD7이 당초 "차단은 M3 소유"라고 적었으나, M3의 Outcome/Scope는 **Reviewer B 부재 fallback**을 다루지 `MCCP_SANTA_BLIND_LANE=off`로 블라인드 레인 자체가 꺼진 경우를 다루지 않는다. 두 축이 겹치지 않으므로 이 상태는 **현재 어느 milestone도 소유하지 않는다**. M1은 완료 조건(live run의 `santa_blind_records >= 1`)으로 `off` 실행을 complete에서 배제할 뿐, 상시 `off` 운용을 막지 않는다. 소유자 후보: M3 범위 확장 vs 신규 milestone. **(갱신 M3, 2026-08-19: M3은 넓히지 않기로 했으므로 남은 후보는 신규 milestone 하나다.)** 거절 근거는 처방이 다르다는 것이다 — 모델 degrade의 처방은 "codex를 설치하라"이고 레인 degrade의 처방은 "`MCCP_SANTA_BLIND_LANE=off`를 그만 쓰라"라, 두 축을 한 verdict에 묶으면 운영자가 받은 정지 메시지가 어느 처방을 가리키는지 흐려진다. 기술적으로는 M3이 만든 봉인 층 강등을 `santa_blind_rounds < santa_rounds`에 재사용하면 거의 공짜이므로, 넓히자는 판단이 서면 그때는 값싸다. 지금 그 결정을 **조용히 내리지 않는 것**이 이 갱신의 목적이다. (Plan-Codex R1 F2 지적으로 등재, 2026-08-18)
...
- [ ] **`--model`은 여전히 선언이라 위조 가능하다** — M3의 PATH 재도출은 *설치되지 않은 CLI의 모델명을 참칭하는* 경로만 막는다. codex가 설치돼 있는데 Claude fallback을 쓰고 `gpt-5.4`라고 적는 것은 막지 못한다 — 셸에서 어느 모델이 실제로 응답했는지 확인할 방법이 없다. M1이 `--lane`에 대해 적은 것과 **같은 천장**이고 M3도 위조 방지를 주장하지 않는다. 검증은 지표 5(두 레인이 동시에 놓친 항목 비율)의 결과 분포에 맡긴다. 관측이 나오면 그때가 이 축을 코드로 좁힐 근거다. (M3 DD6으로 등재, 2026-08-19)
