...
| **diverse-agent review — plan 승인 다관점 전환 (v1.23.1 M1)** | `lib/review-verdict.js`(CREATE — 승인 판독 SSoT) + `lib/plan-review/{l1-check,perspectives,quorum,decide,cli}.js`(CREATE) + `workflows/plan-review.js`(CREATE) + `agents/review-{architect,security,test,invariant}.md`(CREATE) + `receipt/{schema,write,dedupe}.js` + `lib/{receipt-convergence,pr-ship-gate,evidence-audit}.js` + `lib/completion-ledger/{index,store}.js` + `derive/sources/{receipts,worktrees}.js` + `commands/plan.md` — `/mccp:plan` 게이트의 승인 발급자를 **Codex 단독 판정(10~15분·최근 3회 연속 미가용)에서 L1 mechanical + L2 다관점 refute 합성**으로 전환. 승인 표면은 present-only `resolution.review_verdict`/`review_source`/`review_proof` 3필드 신설 + 판정 소비처 전량을 단일 helper `resolveEffectiveVerdict`로 통일(v1.22.5 M1 `receipt-convergence` 수술과 동형, legacy receipt 판정은 byte-동등). **DD2가 이 milestone의 안전 척추** — dedupe skip 술어를 `converged ∧ source ∈ {codex,hybrid}`로 좁혀 multi-agent 승인이 dedupe를 **구조적으로 만족하지 못하게** 했고, 그래서 plan을 패널이 승인해도 terminal `/mccp:pr`에서 PR-Codex가 반드시 발화한다: cross-model은 *제거*가 아니라 반복 지점→ship 지점으로 **이동**했다. **DD11 all-or-nothing**(schema + `write.js` 양쪽 exit 12) — `review_*` 부분 stamp가 `codex_verdict`로 fallback해 dedupe를 통과하던 유일한 구멍을 닫음(부분 stamp는 디스크에 존재 불가). **DD13 bind** — `review_proof.reviewed_plan_hash`를 **L2 발화 직전**(`emit-workflow-args`)에 계산해 봉인 시 `plan_hash`와 대조, 불일치면 exit 12이고 복구는 재봉인이 아니라 **L2 재실행**(본 PRD의 santa-loop R2에서 실제 발생한 실패 모드 — 리뷰어를 띄운 뒤 plan을 편집해 PASS가 편집 전 버전 판정이 됨). **DD3** L1이 gatekeeper(실패 시 L2 미발화·토큰 0) · **DD7** 미상 mode → `codex`(`parseMergedVerifyMode`와 **반대** 방향 — 이 축의 실패는 "검증 꺼짐"이 아니라 "승인 발급자 오인") · **DD8** terminal은 multi-agent 단독 승인 미수용(`multi-agent-unproven-at-terminal`) · **DD9** L2도 runaway cap 소비, `granted:0`은 인라인 강등이 아니라 **HALT**(게이트는 조용히 약해지느니 멈춘다) · **DD10 no-render** derive projection까지만, renderer section 변경 0(mechanical 확인) · **DD6** 신규 6필드 전부 `hash.js` carve-out **미포함**(봉인 대상 — briefing_*이 carve-out인 이유는 봉인 *후* stamp라 자기 자신을 해시 못 하기 때문이고, review_*는 write **시점** 확정이라 그 사유가 없다). read-only 보증은 프롬프트가 아니라 agent frontmatter `tools: [Read, Grep, Glob]` **도구 부재**. Workflow 실패는 fan-out의 fail-open 강등과 **의도적 반대** — 게이트이므로 아티팩트 부재 → `decide` exit 12 → HALT. **후속 `/mccp:code-review`가 command-body seam 결함 8건을 흡수**(구현 검증이 전량 단위라 이 층이 미실행이었다): (a) **모든 게이트 값은 아티팩트 경유** — Bash 블록 간 셸 상태가 보존되지 않고 5.2c의 `Workflow` 호출이 블록을 강제 분리하므로, 전 sub-step이 `REVIEW_DIR`(=`<repo>/.claude/state/plan-review/`, gitignored)를 재파생하고 mode/l1/l2/l3/decision/proof/reservation/started-at를 파일로 주고받는다(§3.9 규칙을 승인 자체에 적용 — 셸 변수 경유였다면 5.6이 review triple을 조용히 미전달해 **승인 기록 0인 receipt**가 성공으로 봉인됐다). 경로는 `.git/mccp/tmp`가 아니라 **repo 루트 하위**여야 한다 — linked worktree에서 `git rev-parse --git-dir`은 working tree 바깥을 가리켜 evidence가 `../..`로 시작하고 `isRepoRelativeEvidencePath`가 거부한다. (b) **plan body 동결(freeze) 불변식** — `emit-workflow-args`부터 receipt write까지 plan 편집 금지. section 추가는 `plan_hash`를 바꾸므로(정규화 대상 아님, 실측) 5.3의 Codex 섹션 주입은 DD13 bind를 **반드시** 깨뜨린다 → panel 모드는 5.3 skip · 5.6 Step A는 mode-aware · 패널 findings는 plan이 아니라 신규 **`.claude/reviews/plan-review-<slug>.md`**(5.2h)에 기록(그 전에는 claim·evidence가 어디에도 도달하지 않아 차단당한 작성자가 근거를 못 봤다). (c) **quorum 관측치 바인딩** — proof 구조 검증이 `perspectives.length === quorum.of`를 요구해 문서화된 default `3of4`가 실질 `4of4`였다(3명 응답 정상 통과가 proof 무효 → `unavailable` 강등). `responded` 기준으로 이전하고 `roles`도 임계값이 아닌 관측치를 기록. (d) **cap 정합** — `emit-workflow-args --granted`가 fleet을 예약분으로 상한하고 `--actual`은 실제 emit된 `fleet.length`(이전엔 granted<4여도 4개를 띄우고 granted만 기록). payload의 `fleetKeys` 누락으로 workflow가 **매 실행** 리뷰어 1명으로 degrade하던 것도 함께 수정. **origin/main(#118 codex-intent-context M1) reconcile — intent gate carve-out.** #118은 `mccp-plan-codex` receipt의 writer를 `plan-codex-runner.js` 단독으로 좁히고 PRD-모드 plan에 **프로그래매틱 `intentDecision`을 강제**한다(`--intent-*` CLI 플래그는 위조 방지를 위해 의도적으로 0건). panel 경로는 Codex를 호출하지 않아 runner가 없으므로 **default mode가 PRD-driven plan에서 receipt를 아예 쓸 수 없게 되는** 충돌이 생겼다. 해소는 main 자신의 **DD1 free-form carve-out을 미러링**한다 — `write.js#stampIntentDecision`이 `resolution.review_source === 'multi-agent'`를 보고 `intent_gate_verdict='skipped'` + 신규 proof `intent_skip_proof='codex_not_invoked'`를 stamp한다. 이는 판단이 아니라 **기계적 사실**이다: review triple은 all-or-nothing(DD11)이고, proof는 구조 검증을 거쳤으며, `multi-agent`와 `codex_verdict`를 동시에 담은 receipt는 이미 거부된다 — 즉 Codex가 발화하지 않았음이 봉인 대상 receipt 자체로 증명된다. **`hybrid`는 제외**(L3 발화 = Codex 발화 → adjudication 의무 존속, fail-closed 유지)이고 review 축이 없는 bare write도 그대로 차단된다. dedupe는 **무변경** — DD2가 panel을 cross-model 확증으로 이미 거부하므로 이 intent stamp가 PR-Codex skip을 사지 않는다(전용 회귀 `plan-review-intent-carveout.test.js`가 carve-out·경계·DD2 불변·두 enum lockstep 4축을 고정하고, 변이 검증으로 실효성 확인). **알려진 갭(M1.5)**: 패널은 `<user_intent_reference>` 주입을 받지 않고 자기 findings를 user intent에 대해 adjudicate하지 않는다 — 패널 run에서 intent gate는 *만족*된 것이 아니라 *skip*된 것이다. 버전은 §3.7 forward-only로 `1.23.1 → 1.23.5`(main 1.23.4 위). M2(L3 자동 트리거)·M3(implement-verify 3층 확장)·M1.5(패널 intent 편입) 이연. | v1.23.5-m1 ship |
