...
| 2 | implement 병렬화 | dispatch-controller를 Workflow primitive로 리팩터, N-worker 병렬 구현, worktree 격리 표준화. commit/PR 격리 invariant 유지. **M2b Task 0 spike 실측**: `isolation:'worktree'`→parent 자동 merge 미입증 → merge_strategy=disable-parallel로 병렬 스캐폴드는 완성하되 실행은 N=1 gate off. **M4(v1.21.0)가 live 상관 입증 후 default를 worktree-merge로 flip → 이 스캐폴드가 런타임 활성화됨** | complete | M2a `.claude/plans/workflow-orchestration-m2a-single-worker-workflow.plan.md` · M2b `.claude/plans/workflow-orchestration-m2b-nworker-parallel.plan.md` |
| 3 | verify 네이티브화 (+ worktree-merge 활성화) | **verify 축 SHIPPED (v1.20.12)** — 통합 diff aggregate adversarial-verify를 `/mccp:work` Step 3.verify 필수 pipeline 스테이지로 강제(cross-model Codex 유지, DD2). **DD6: 단일 경로에서도 발화**하므로 병렬 gated여도 runtime 가치 확보. worktree-merge substrate(collect/apply/patch-scoped rollback + 신규 gate `mccp-implement-verify`)는 build+unit-test 완비. worktree-merge 활성화 축은 M3에서 GATED(honest degradation, DD7 — live 상관 cost hard-ceiling 미실측)였으나 **M4(v1.21.0)에서 live 입증·해금 완료** — 이 milestone의 verify 축 + 이연했던 활성화 축 모두 종료 | complete | `.claude/plans/workflow-orchestration-m3-verify-native.plan.md` |
| 4 | 병렬 활성화 (worktree-merge live) | **SHIPPED (v1.21.0)** — Task 0 live dogfood(run wf_1f689994-fb8 topology + wf_98047bb7-1b1 happy-path)가 worktree↔dispatchId 상관을 empirical 입증: isolation:'worktree' worktree는 `<repo>/.claude/worktrees/wf_<runId>-<N>`에 생성·`parallel()` 반환 후 컨트롤러 enumerable·잔존, worker가 first-step seed한 envelope를 collect-worktrees가 correlate(`.claude/state/dispatches/` gitignored→parent 미복사=seed-required). `merge_strategy` default를 `worktree-merge`로 flip → seed→mark→collect→reconcile(worktree-read)→merge-apply→rollback 전 체인 live 완주. cost guard 3중(opt-in·cost-state·tier autoDisable) 무변경. 신규: `seedEnvelope`+`seed-envelope` 서브커맨드, reconcile worktree-envelope read, merge-apply patches-out rollback(F1), seed/mark worktree-root 정규화(F2), collectChangedFiles `-uall`(dogfood-surfaced) | complete | `.claude/plans/workflow-orchestration-m4-parallel-activation.plan.md` |
