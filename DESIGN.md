...
### Navigation
- **Sidebar** (`.sidebar`, 244px sticky full-height): 프로젝트 스위처 + **검색 입력(M3 wiring 완료 — 실제 `<form role="search">` + `<input type="search">`, 아래 Search 절)** + page nav 레일 + 차단 pin-alert. sidebar-black 배경 + 우측 hairline. **필터·정렬 바는 사이드바가 아닌 위험·질문 패널 head 에 통합**(M2 panel-header canonical — 아래 Filter/Sort 절) — 레일은 wayfinding + 전역 검색만 담아 nav 무게감·키보드 탭순서를 청정하게 유지한다(검색은 global wayfinding 이라 사이드바가 scope 일치).
...
- **필터·정렬 컨트롤 (Data Exploration M2):** 위험·질문 항목에 **필터(PRD축·plan축, AND 조합)** + **정렬(위험도순·시간순)** 컨트롤 바(`.explore-bar`)를 더한다. 컨트롤은 `.js-only`라 JS-off 시 사라지고 전체 항목이 그대로 보인다(PE 불변). 항목엔 `data-plan`(plan 필터 키)·`data-sev`(정렬 키)·`data-ord`(원본 parse chronology — severity 정렬 *이전* 순서라 "시간순"이 severity 를 인코딩하지 않음) 속성. 순수 필터/정렬 로직은 UMD 모듈(`parsers/explore-sort.js`)로 분리해 node 테스트와 browser inline 이 공유한다. 정렬 scope 는 `.stack-list` 단위(그룹 경계 보존). 결과 수는 `aria-live="polite"` 로 알리고, 전체 0 이면 빈 상태 메시지.
  - **컨트롤 형태 (UI/UX, 레퍼런스: GitHub Issues · Linear · Vercel):** native `<select>`/`<button>` 표준 affordance(커스텀 드롭다운 reinvent 금지 — product ban). 원칙: (1) **PRD·plan select 폭 고정(`12rem`)** — 옵션 내용 길이와 무관하게 위험·질문 패널이 동일 형태(native select 가 widest option 으로 auto-size 해 패널마다 폭이 달라지던 inconsistency 차단). (2) **결과 수는 별도 텍스트가 아니라 패널 탭(미해결/완화/해결)의 `.tab-count` 를 엔진이 갱신**(미해결 18→8) — 리스트 바로 위 탭에 반영돼 시각 표면이 깨끗(컨트롤 cluster 에 끼지 않음). `.explore-count` 는 `.sr-only` live-region 으로 스크린리더에게만 "N개 표시" announce(a11y 보존). (3) **focus-visible `outline-offset:1px` + gap `0.5rem`** — 인접 컨트롤로 outline 침범 차단. (4) **컨트롤은 한 줄 고정(`flex-wrap:nowrap`)** — 정렬이 둘째 줄로 떨어지는 2-tier 방지(좁으면 바 전체가 제목 아래 한 줄로). 필터군(`.ex-filters`) ↔ 정렬 간격 분리(Linear 패턴). **초기화는 항상 노출**(현대 필터 UI 필수 affordance).
  - **배치 결정 (impeccable critique + 사용자 확정 2026-06-26):** **패널 헤더 통합**을 단일 canonical 로 채택한다 — 각 컨트롤 바가 자기 위험·질문 패널의 `panel-head`(제목·count 줄) 우측에 통합돼, 컨트롤이 제어 대상 리스트 바로 위에 산다(`scope=배치 일치`). 초기 *전역 사이드바 배치* 는 scope↔placement 불일치(5 route 중 위험·질문 2개만 제어 + 나머지 route 에선 inert chrome), 위험·질문 옵션 결합으로 인한 cross-route 빈 상태, nav 레일 무게감, 키보드 탭순서가 필터 컨트롤을 페이지 nav 보다 먼저 통과하는 비용 때문에 폐기했다. dual-path 토글(`MCCP_EXPLORE_CONTROL_PLACEMENT`)도 함께 제거 — 패널 head 통합이 유일 경로다. 각 패널 바는 자기 route 옵션만 소비하고(`scope='route'`, `closest('.route')`), 한 `.li-item` 집합당 컨트롤러는 1개다(Codex F3 단일 컨트롤러 불변 유지).
- **검색 + 멀티세션 잔여축 (Data Exploration M3):** PRD ③의 마지막 마일스톤이 세 표면을 닫는다.
  - **검색(cross-route 가시성 모델):** 사이드바 검색 입력이 **문서 전역 `.li-item`**(위험·질문)을 헤더/요약(`.li-main`) 텍스트로 동시에 좁힌다(150ms debounce, 단축키 0). 검색은 **가시성(`hidden`)만 토글하고 라우팅·DOM 위치는 미변경**(DOM-move 0) — `:target` 단일-뷰 라우팅과 충돌하지 않는다. 한 번에 한 route 만 보이되, nav-link 매칭 뱃지(`.nav-search-count`) + 전역 `aria-live` live-region("전체 N개 일치 · 위험 8 · 질문 2")이 cross-page 매칭을 surface 해 점프(기존 nav)를 돕는다. 매칭 범위는 `.li-main`(헤더/요약)만 — 접힌 드로어 detail 은 제외(안 보이는 텍스트 매칭 노이즈 회피).
  - **가시성 reason 모델(검색 ↔ 필터 AND 합성):** 한 `.li-item`의 최종 가시성 = `!(_hf || _hs)`. `_hf`=explore-bar 필터, `_hs`=검색 — 두 컨트롤러가 각자 자기 reason expando 만 set 하고 공유 `recompute`가 `hidden`을 합성한다(독립 필터 AND 표준 패턴 — 같은 `hidden`을 두고 경쟁 0). expando(`_hf`/`_hs`)라 DOM attribute 미사용(lint 표면 0). **검색 폼은 `submit` → `preventDefault` + `action`/`method` 미지정**으로 Enter-navigate(route·필터·검색 상태 손실)를 차단한다(Codex Plan-F1). `data-js="on"` 은 `window.__mccpExplore` 확인 *뒤* 에만 set 해, 의존 스크립트 누락 시 `.js-only` 컨트롤이 보이지만 inert 가 되는 dead-UI 를 막는다(Codex Implement-IF1).
  - **멀티세션 잔여축:** `#route-activity` 멀티세션 테이블에 **진행상태·worktree 필터 + 진행순 정렬** 바(`buildSessionBar`, `data-explore-scope="session"`)를 통합한다 — buildExploreBar 와 동일 chrome·neutral 토큰 재사용. 행엔 `data-status`/`data-worktree`(안정 키)/`data-progress-rank`(blocked3>degraded2>active1>idle0, `KIND_META.rank` SSoT)/`data-activity-ord`(recency index) 속성. 진행순 정렬은 rank desc + activity tie-break 로 `<tbody>` 행을 재배열한다. 세션 바는 M2 리스트 컨트롤러와 셀렉터로 **소유권 분리**(`:not([data-explore-scope="session"])` vs `[data-explore-scope="session"]`) — 이중 바인딩·무관 카운트·무효 sort reset 차단(Codex Implement-IF2). 단일 healthy worktree 면 테이블 자체가 graceful-hide 라 바도 동반 미렌더(dead control 0). **작업범위순 정렬은 PRD 명시대로 보류**('PRD 기준 작업 진행도' 재기획 전까지 미노출).
