...
> **§3.7**: `1.34.2 → 1.34.3` (**patch** — orchestrator-step-wiring PRD의 단일
> milestone M1이고 PRD 종료 축이 아니다). **초판 plan은 `1.33.8`을 잠정 target으로
> 적었으나 그 전제가 stale했다**: plan 작성 시점의 origin/main은 `1.33.7`이었는데 이
> 브랜치가 사는 동안 `1.34.1`까지 발행됐다. 발행된 번호는 불가침이라 그 위로 밀면서,
> sibling worktree `c1-review-record-linkage`가 선언 중이던 `1.34.2`와의 충돌을 피해
> 한 칸 더 올려 뒀는데 — base 머지 시점에 그 `1.34.2`가 실제로 origin/main에 발행돼
> (`a9fa92f`) 예측이 실측으로 확정됐다. §3.7대로 `/mccp:pr` 진입 직전 재계산했고
> `1.34.3`은 여전히 미발행이라 그대로 유지한다.
