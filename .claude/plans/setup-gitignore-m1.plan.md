...
- **Action**:
  ...
  - `applyMerge(target, plan)` → `settings-writer.js:36-57` writeAtomic 골격(`.tmp` write → 기존 파일 `.bak` 회전 → rename, fallback copy+unlink)에 **lost-update 방어 3종을 얹는다**(Plan-Codex R5 F1). 실패는 전부 **throw**(DD1).
    ...
    **동의 없는 전체 파일 교체 경로를 하나도 남기지 않는 것이 이 설계의 핵심이다**(Plan-Codex R8 F1 → R9 F1에서 완결).
    ...
    | `update` | **기본은 쓰지 않는다** — `action:'update-required'` 보고 + exit 0. 실제 재작성은 **`--force-update`를 준 경우에만** | 동의 없는 재작성이 존재하지 않는다 |

    **`update`를 명시 동의로 돌린 것이 UI2를 실제로 충족시키는 지점이다.** 이전 초안은 `update`의 잔여 창을 "드물다"며 알려진 한계로 기록했는데, UI2는 "기존 줄은 절대 변경·삭제하지 않는다"는 **무조건 제약**이므로 드묾은 충족이 아니다(R9 F1). 동의 없는 전체 교체 경로를 없애면 남는 재작성은 사용자가 그 순간 명시적으로 요청한 것뿐이고, 그 전제에서는 사용자가 동시에 편집 중이 아니다. 대가는 정본이 바뀐 버전에서 한 번의 명시 동작이며, 사용자 줄 유실 가능성보다 명백히 싸다. 아래 3종 방어는 **`--force-update` 경로에만** 적용된다.
    ...
    **원자적 rename은 lost update를 막지 못한다.** rename이 보장하는 것은 "부분적으로 쓰인 파일이 보이지 않는다"까지이고, `nextContent`가 이미 낡았다면 그 낡은 전체 내용이 원자적으로 착지할 뿐이다. 사용자가 read 이후 `.gitignore`에 줄을 추가했다면 그 줄은 **조용히 사라진다** — UI2("기존 줄은 절대 변경·삭제하지 않는다")의 정면 위반이고, 순차 재실행만 다루는 test로는 영원히 보이지 않는다.

    **남는 창의 범위 — `--force-update`를 준 실행 안에서만**. `<target>.lock`은 mccp writer끼리만 협력시키고 임의 편집기의 쓰기를 막지 못하며, 그것을 막는 이식 가능한 배타 잠금 원시가 Node에 없다(Windows와 POSIX에서 의미가 다르다). 따라서 재검사와 rename 사이의 창 자체는 제거되지 않는다. 달라진 것은 **그 창에 들어가려면 사용자가 그 순간 명시적으로 재작성을 요청해야 한다**는 점이다 — 자동 실행이 조용히 밟는 경로가 아니다.

    - 재검사는 **rename 직전에 배치**해 창을 최소화하고, `.bak`이 직전 상태를 보존한다.
    - 기본 실행(`--force-update` 없음)에는 전체 교체가 **존재하지 않으므로** 창도 없다.
    - 따라서 이 plan은 UI2를 "흔한 경로에서만"이 아니라 **동의 없는 모든 경로에서** 충족한다고 주장한다. 사용자가 명시 요청한 재작성의 마이크로초 창은 UI2가 금지한 "mccp가 사용자 줄을 바꾸는 것"과 성격이 다르다.
  ...
  - CLI: `node gitignore-provision.js provision [--dry-run] [--repo <path>] [--force-update] [--json]`. **`--repo` 값(없으면 `process.cwd()`)을 `resolveRepoRoot`의 `cwd` 인자로 그대로 전달한다** — 이 배선이 `--repo`가 실제로 대상을 바꾸는 유일한 경로다(R5 architect MEDIUM: 배선을 산문 밖에 두지 않는다). `require.main` 블록 전체 try/catch → 성공 exit 0 / 오류 stderr + exit 1 / **`not-a-git-repo`만** `action:'skip'` + exit 0 / `git-unavailable`·`git-error`·`marker-damaged`는 exit 1 (DD1). `--dry-run`은 `addedLines`를 출력하고 쓰지 않는다(UI9). 쓰기 대상 경로는 **항상 `resolveRepoRoot`의 반환값**이며 `--repo` 원문이 아니다(worktree/하위 디렉토리에서 호출해도 repo 루트에 쓰기 위함).
  - **stdout JSON 스키마** (Task 3이 파싱하므로 명시 고정):

    | 필드 | 타입 | 설명 |
    |---|---|---|
    | `ok` | boolean | 성공 여부. `false`는 exit 1과 항상 동반 |
    | `action` | `'create'｜'append'｜'update'｜'update-required'｜'noop'｜'skip'` \| null | `skip`은 not-a-git-repo (exit 0). **`update-required`는 정본이 바뀌었으나 `--force-update`가 없어 쓰지 않았음**(exit 0, UI2). 오류 경로(`ok:false`)에서는 **null** |
    ...
    | `backupPath` | string \| null | `.bak` 경로. **`--force-update` 전체 교체에서만 non-null**이며 `create`/`append`/`dryRun`/`noop`/`skip`/`update-required`는 모두 null (R12 F1) |
...
- **Action**: `gitignore-provision.test.js`에 다음 케이스. 전부 `mkdtempSync` 격리 디렉토리에서.
  ...
  - **실제 write E2E** (dry-run만으로는 검증 안 되는 축)
    - 임시 repo에 `git init` → `provision` 실행 → 파일에 블록 존재 확인 → **재실행** → `action:'noop'` + 파일 mtime/내용 불변. **`.bak`을 단언하지 않는다** — `create`/`append`는 전체 교체를 하지 않으므로 백업 대상이 없다(R11 F2). `.bak` 단언은 `--force-update` 경로 test 전용이다.
    ...
    - **`--force-update` 경로 전용**: 정본이 바뀐 상태를 만들고 `--force-update`로 실행 → 전체 교체가 일어나며 이때만 `.bak`이 생기고 **실행 전 파일과 바이트 동일**함을 단언(R3 test HIGH의 요구를 이 경로로 이관).
  ...
  - **`.bak` 내용 동일성** (R3 test HIGH → R12 F1로 경로 한정): **`--force-update` 실행에서만** `.bak`이 생기며, 그때 **실행 전 파일과 바이트 동일**함을 단언한다(존재만이 아니라 내용까지). `.bak`은 전체 교체 시 사용자 파일 복구의 유일한 수단이므로 존재 확인만으로는 부족하다. **`create`/`append`에서는 `.bak` 미생성을 단언한다** — 전체 교체를 하지 않으므로 백업 대상이 없고, 여기서 `.bak`을 요구하면 쓰기 계약과 모순되어 구현이 불가능해진다.
...
- **Action**: Phase 4 뒤에 `## Phase 5 — Provision .gitignore` 삽입, 기존 최종 보고를 `## Phase 6 — Final report`로 이동.
  ...
  - 본문 계약(DD1을 명령 본문에 그대로 씀):
    ...
    - `noop` → "이미 최신" / `create`·`append` → 추가된 줄 수(**`.bak` 없음** — 전체 교체를 하지 않는다) / `update`(`--force-update`) → 갱신된 줄 수 + `.bak` 경로 / `update-required` → 재작성이 필요하나 동의 없이 쓰지 않았음을 안내.
  ...
  - **Phase 5의 bash 블록은 아래를 그대로 쓴다.** exit-code 전파를 산문으로만 요구하면 "provision을 호출은 하되 exit status를 무시하는" 구현이 통과한다(R2 invariant CRITICAL). 계약을 실행 가능한 코드로 못박는다:
    ...
    case "$PROVISION_ACTION" in
      ...
      create|append|update)
        # backupPath는 --force-update 전체 교체에서만 non-null이다. create/append는
...
- **Validate**: 명령 본문은 markdown이라 실행 E2E가 불가능하다 — **그 한계를 인정하고 계약의 구조를 정적으로 단언**한다.
  ...
  **이 계약 검사의 단일 소유처는 Task 2의 `setup.md` 계약 lint(test 파일 안)이며, Task 5가 그 test 파일을 CI에 등록한다**(R3 test MEDIUM — 세 곳에 흩어져 어디가 정본인지 모호했던 것을 여기서 확정). 아래 목록은 그 lint가 검사할 **항목의 정의**이고, 수동 실행용 사본이 아니다:
  ...
  | 10 | `case "$PROVISION_ACTION" in` 존재 **및** `skip`/`noop`/`create`/`append`/`update`/`update-required` 6개 action 이름이 모두 등장 — 방어 구조만 남고 보고 분기가 사라지는 경우를 잡는다(R5 invariant MEDIUM) |
...
- [x] `.bak`은 **`--force-update` 경로에서만** 생기며 그때 실행 전 파일과 **바이트 동일**함이 test로 확인됨. `create`/`append`에서는 **미생성**임을 단언 (R3 test HIGH → R11 F2로 경로 이관)
...
- [x] **`--force-update` 없이는 전체 파일 교체가 일어나지 않음**이 test로 확인됨 — 정본이 바뀐 상태에서 기본 실행은 `action:'update-required'` + exit 0 + **파일 무변경** (UI2 충족의 핵심, Plan-Codex R9 F1)
...
- YAGNI Triage:
  ...
  | F15 — `--force-update`도 UI2를 위반한다 | HIGH | **REJECTED_BY_DESIGN** | 전제 불성립. marker 구간의 줄은 **mccp가 쓴 mccp의 줄**이지 사용자의 줄이 아니며, 같은 PRD의 UI3(사용자 규칙을 정리하지 말 것)이 그 구분의 근거다. 자기 블록 갱신을 사용자 줄 변경으로 읽으면 PRD가 요구한 멱등 병합 자체가 정의상 불가능해진다. 남은 절반인 TOCTOU 잔여는 실재하나 OS 수준 배타 잠금 부재로 이 계층에서 닫히지 않아 backlog 이관 |
  | F16 — `.bak` 단언이 create/append 쓰기 계약과 모순되어 구현 불가 | MEDIUM | **ACCEPT_NOW** | 직전 흡수가 만든 모순. `.bak`을 `--force-update` 전용으로 옮겨 놓고 create/append E2E는 여전히 `.bak`을 단언해, 계약대로 구현하면 test가 실패하고 test를 맞추면 명세에 없는 백업·전체 교체를 추가해야 했다. `.bak` 단언을 `--force-update` 경로로 이관하고 create/append는 **미생성**을 단언하도록 정정 |
  | F14 — 병렬 append가 managed 블록을 영구 중복시킴 | HIGH | **ACCEPT_NOW** | 직전 흡수가 만든 회귀. 방어를 `--force-update` 전용으로 국한하며 **lock까지 함께 옮긴 것**이 원인이다. lock은 전체 교체를 보호하는 장치가 아니라 mccp writer를 직렬화하는 장치이므로 `create`/`append`/`update` 전 경로에 있어야 한다. 없으면 두 writer가 각자 블록을 붙여 이후 실행이 `damaged`로 영구 파손되고, 병렬 test 불변식(양쪽 exit 0 · 후행 `noop`)과도 모순된다. lock을 전 경로로 되돌리고, `--force-update` 전용은 해시 재검사와 전체 교체만으로 국한 |
  | F12 — 모든 managed-block update에서 UI2가 미충족으로 남음 | HIGH | **ACCEPT_NOW** | 드묾은 충족이 아니라는 지적을 받아들여 잔여를 설명하는 대신 **제거**했다. `update`를 명시 동의(`--force-update`) 경로로 돌려 **동의 없는 전체 교체가 존재하지 않게** 했다. 기본 실행은 `action:'update-required'` 보고 + 파일 무변경. 직전 라운드의 `intent_override_reason`은 이 라운드에서 불필요해졌다 |
