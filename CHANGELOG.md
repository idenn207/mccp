...
- Tests: `receipt/tests/block-format.test.js`(신규 8) · `hooks/tests/receipt-prompt-tamper.test.js`(신규 3) · `preflight.test.js`(+1 subject-tamper) · `lib/tests/history-leak-scan.test.js`(+2 F1: base-blob-new-path + ancestor-only-deleted-before-HEAD). 버전은 1.22.6 유지(미머지 M2에 리뷰 흡수).
