...
> **§3.7**: `1.30.2 → 1.30.3` (**patch** — M7은 multi-session-work-loop PRD의 단일
> milestone이고 M8이 아직 pending이라 PRD 종료 축이 아니다). **번호를 두 번 상향했다**:
> 구현 시점에는 origin/main·브랜치가 모두 `1.30.0`이라 `1.30.1`을 잡았으나, `/mccp:pr`
> 진입 직전 재계산에서 origin/main이 이미 `1.30.1`(codex-intent-context M2)과
> `1.30.2`(diverse-agent-review M7)를 발행한 상태였다. §3.7의 forward-only 상향에 따라
> 발행된 번호는 불가침으로 두고 미머지 브랜치 쪽만 밀어 `1.30.3`에 착지한다. 4면(plugin.json ·
