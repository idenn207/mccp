...
- **알려진 선재 결함(본 커밋 무관, backlog).** 5.6의 `--codex-verdict`는 `$CODEX_VERDICT` 셸 변수에서 오는데 Bash 블록 간 셸 상태는 보존되지 않는다(Codex 지적). `git show`로 확인한 결과 이 의존은 **v1.23.0 이전부터 존재**했고 본 커밋은 그 앞에 `FORWARD_CODEX` AND를 더했을 뿐이다. 방향은 fail-closed(verdict 부재 → dedupe가 막혀 PR-Codex 발화)라 안전 측이므로 별 사이클로 이연한다.
