...
분모는 단조 증가하고, 임계 통과는 확실하다. 오늘 3이다(santa R3 재측정 —
`node plugins/mccp/scripts/lib/msw-metrics/cli.js a1`. 이 문서는 처음에 23이라 적었고
그 수치는 배너가 재는 값과 어긋났다. 임계까지의 거리가 실제로는 더 멀다는 뜻이며,
시한폭탄이라는 판정 자체는 단조 증가라는 성질에서 나오므로 바뀌지 않는다).
**A1은 통계적으로 의미를 갖기 시작하는 바로 그 지점에서 `status:'invalid'`로 죽는다.**
...
- **필드를 싣는 것만으로는 미완이다 — 렌더 경로를 함께 연다.** 근거는 아래처럼 정정한다
  ...
  이 계획 자신이 startupCount는 오늘 3이라고 적으므로 `startupCount > 50` 가드는 애초에
...
- **Validate**: `startupCount=51` · 기준선 부재 → `status='computed'`(오늘 `invalid`) ·
  ...
  나타난다 — `startupCount`가 임계 아래(예: 3)일 때도 나타나야 한다. 51에서만 단언하면
...
- [ ] **A1 가드의 가시성**: 잠든 가드가 `cli.js a1` 출력에 실제로 나타난다 — `startupCount`가
      임계 아래(오늘 3)일 때도 나타난다. 51에서만 나타나면 오늘 도달하지 않는 조건만 검사한
