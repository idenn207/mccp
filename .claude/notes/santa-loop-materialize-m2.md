...
| 항목 | plan 지정 | 착지 |
|---|---|---|
| M1 (선행 커밋) | `1.23.8` | `1.25.2` (patch) |
| M2 (본 milestone) | `1.23.9` | `1.26.0` (minor — PRD 종료) |

M1은 두 번 밀렸다. 1차 상향에서 `1.25.1`을 잡았으나 PR 제출 전에 main이 **같은 번호를 diverse-agent-review M6**(PR #138, merge `890f725`)에 발행해 중복이 됐고, 발행분 불가침 원칙에 따라 미머지 쪽인 M1만 `1.25.2`로 한 칸 더 올렸다(§3.7 9번째 재발). M2의 `1.26.0`은 main 최대치 `1.25.1`보다 여전히 앞서므로 무변경이다.
