...
test('html — footer version v1.23.2 (footer element anchored)', () => {
  const r = renderWithStubs(makeFullModel(Date.now()));
  ...
  assert.match(r.html, /<footer[^>]*>v1\.23\.2 ·/);
...
test('markdown — footer with v1.23.2 version (footer line anchored)', () => {
  const r = renderWithStubs(makeFullModel(Date.now()));
  ...
  assert.match(r.md, /^_derived from \.claude\/ · v1\.23\.2_$/m);
});
