...
| G3 | **모든 덮어쓰기는 덮어쓴 쪽이 보고하고 감사에 남는다** | heartbeat 연장 lease → post-rename **소유 재확인** → `evidence_overwrite_observed` 기록 + fail-closed throw + B2 런타임 감사의 pre-hash 불일치 검출 |
...
- **덮어쓴 쪽(A)은 반드시 보고한다** — A는 rename **후**에도 lock 소유를 재확인하고, 자기 token이 이미 lock에 없으면 "나는 소유 없이 썼다"를 확정 인지해 `evidence_overwrite_observed` + fail-closed throw. 죽은/정지한 holder는 애초에 rename에 도달하지 못하므로, 이 경로를 타는 것은 **살아서 재개한 A뿐**이고 A는 항상 보고 가능하다.
