...
test('html — footer version v1.20.6 (footer element anchored)', () => {
  const r = renderWithStubs(makeFullModel(Date.now()));
  ...
  assert.match(r.html, /<footer[^>]*>v1\.20\.6 ·/);
...
test('markdown — footer with v1.20.6 version', () => {
  const r = renderWithStubs(makeFullModel(Date.now()));
  assert.match(r.md, /v1\.20\.6/);
});
