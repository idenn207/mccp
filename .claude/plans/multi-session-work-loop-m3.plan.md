...
| G2 | **stale·부활 holder의 write-time 거부** | `{session_id, host, pid}` 3원소 holder 대조 + release tombstone(TTL 창 안) |
| G3 | **어떤 증거 손실도 조용히 지나가지 않는다** | heartbeat 연장 lease → post-rename 검증 → `evidence_overwrite_observed` 기록 + fail-closed throw |

**G3은 "lost update가 불가능하다"가 아니다.** `rename`은 advisory lock에 대한 CAS가 아니므로 reclaim↔rename 창은 원리상 남고, santa-loop 두 리뷰어가 이를 독립 확인했다. M3은 그 창을 **감지·차단**하며, PRD의 B2가 세는 것이 "덮어쓴 **사고 건수**"이므로 감지되어 게이트를 중단시킨 경합은 사고로 계상되지 않는다. 무조건적 상호배제는 파일시스템 원자 CAS 또는 단일 writer 프로세스를 요구하며 **M3 범위 밖**이다.
...
**enforcement locus는 command body가 아니라 write path다.** LLM이 지시를 건너뛰어도 receipt write는 반드시 `writeReceipt`를 지나므로(store.js:112-121이 `assertNoTrackedOverwrite`에 대해 이미 같은 논거를 세움), 보호가 모든 caller에 자동 적용된다. command body 변경은 조기 경고(UX)일 뿐 enforcement가 아니다.
...
- **Action**: 키 = decision slug(PRD 작업단위 freeze = milestone = plan = PR = slug). `acquireClaim({slug, sessionId})` → `.claude/state/evidence-claims/<slug>.json`에 `{slug, session_id, host, pid, claimed_at, claim_epoch}` 원자 생성. **live 판정은 `listLedgers({activeOnly:true})` 재사용** — heartbeat TTL·PID liveness·host 분기가 이미 검증된 substrate이므로 만료 의미론을 새로 만들지 않는다. 동일 `session_id` 재진입은 **멱등 no-op**(단일 세션 사용성 무해 — PRD Risk 대응). 다른 **live** 세션 보유 시 `{ok:false, holder}` 반환. holder가 ledger에서 inactive면 자동 승계(+ 승계 이벤트). **advisory↔enforce**: 코스한 착수 차단은 advisory 통보, enforce는 write-side fencing만 — CoAgent 근거(차단형 락보다 advisory가 처리량 1.4×) + PRD Risk "차단은 실제 충돌 시에만".
...
- **(santa R1 I3 — Reviewer B) 세션 정체의 부재·중복 분기를 명시한다**: B가 지적한 대로 "실행 세션 정체"만으로 대조하면 두 취약점이 남는다. (a) **정체 부재** — `CLAUDE_SESSION_ID`가 없는 프로세스가 두 개면 둘 다 "정체 없음"으로 같아져 서로를 자기 자신으로 오인한다. (b) **정체 공유** — 같은 세션 id를 가진 두 프로세스가 하나의 holder로 붕괴한다. 해소: holder 레코드의 정체를 **`{session_id, host, pid}` 3원소**로 기록하고 **세 값이 모두 일치할 때만** 동일 holder로 본다. `session_id` 부재 시 프로세스별 nonce로 대체해 두 무명 프로세스가 우연히 같아지지 않게 한다. 정체가 붕괴한 잔여 경우(같은 host·pid까지 같을 수는 없으므로 실질 0)라도 I1의 post-rename 검증이 **마지막 그물**로 남는다.
...
- **Action**: `session-start.js` — `listClaims()` × `listLedgers({activeOnly:true})` 교차로 **다른 live 세션이 점유한 작업 단위**를 `<system-reminder>`에 주입(차단 없음). 모듈 로드 실패에도 세션 부팅 무중단(fail-loud-open). `work.md` Step 0 — 착수 slug의 점유를 조기 확인해 사용자에게 알린다. **본문에 "enforcement locus는 receipt write path이며 이 단계는 조기 경고"임을 명시** — LLM이 이 단계를 건너뛰어도 보호는 유효하다는 계약을 문서화(command body 게이트 신뢰 금지).
...
node --test plugins/mccp/scripts/lib/tests/evidence-claim.test.js      # (R1F3) A만료→B승계→B release→A부활 → stale epoch 거부
...
- [ ] **(santa R1 I3) 정체 분기 전수** — holder 정체가 `{session_id, host, pid}` 3원소이고 셋 모두 일치할 때만 동일 holder · `session_id` 부재 프로세스 2개가 서로를 자기 자신으로 오인하지 않음 · 동일 `session_id` 공유 2 프로세스가 하나로 붕괴하지 않음 · **tombstone TTL 만료 후 fence lapse가 known-gap test로 고정**(조용한 동작 변화 감지)
...
| I3 | B | 세션 정체 부재/공유 분기 미정의 · tombstone TTL 만료 후 재개방 | holder 정체 `{session_id, host, pid}` 3원소 + 무명 프로세스 nonce + **TTL lapse를 known-gap test로 고정** |
