...
### Navigation
- **Sidebar** (`.sidebar`, 244px sticky full-height): 프로젝트 스위처 + 검색 affordance(현재 `aria-hidden` 시각 placeholder — 검색 wiring 은 M3) + page nav 레일 + 차단 pin-alert. sidebar-black 배경 + 우측 hairline. **필터·정렬 바는 사이드바가 아닌 위험·질문 패널 head 에 통합**(M2 panel-header canonical — 아래 Filter/Sort 절) — 레일은 순수 wayfinding 만 담아 nav 무게감·키보드 탭순서를 청정하게 유지한다.
