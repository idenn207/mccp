...
**영속성 — 이 토글은 만기를 갖지 않는다** (orchestrator-step-wiring M3 · DD5). 임시 스위치가
아니라 **되돌림 수단**이다. M1이 A1 축의 기록 위치를 옮긴 것은 되돌릴 수 있어야 하는 종류의
결정이고, 그 되돌림을 코드 롤백이 아니라 값 하나로 할 수 있게 남겨 둔 것이 이 변수의 존재
이유다. 따라서 "언제 제거하는가"에 대한 답은 **제거하지 않는다**이며, 미설정 상태로 오래
지났다는 사실은 은퇴 근거가 되지 않는다 — 그것은 기본값이 맞았다는 뜻이다.

같은 이유로 이 항목은 `retired.md`로 옮기지 않는다. 은퇴 판정의 통상 신호(소비처 소멸 ·
대체 축 도입)가 성립하려면 A1 축의 공유 기록 자체가 철회돼야 하는데, 그때 필요한 것이
바로 이 토글이다.
