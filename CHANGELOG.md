...
### Fixed — PR-Codex R1 6라운드 + Implement-Codex R1 7라운드 흡수 (4건 전부 ACCEPT_NOW, backlog 이연 0)
...
- **debt decay** — 위 pin이 만든 새 trade-off를 같은 사이클에서 답한다. 5라운드 debt는 pending을 **영구** 고정해 lease가 막으려던 자기중독을 재도입했다(pin 후 죽은 컨트롤러 = headroom 영구 소진). `MCCP_ORCHESTRATION_DEBT_DECAY_HOURS`(default 6, `cost-state.js#decayIfStale` 미러) 경과 시 마커가 pin을 놓아 오차가 **보수적 over-count로 시작해 자기치유**한다. `readCounter`/`reconcileReservation`이 동일 env/now로 같은 판정 공유(불일치 시 A reconcile이 B의 pin을 재해석 — test 검증). **`0`은 유효한 kill switch**(lease의 0과 대조 — lease는 0이 자기중독 복원이라 거부, debt는 0이 보수적 선택이라 정당). read-side는 뷰에서만 제외하고 unlink 안 함(read-only 불변식 — 디스크 잔존 test).
