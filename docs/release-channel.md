...
**`source.url === 'https://github.com/idenn207/mccp.git'`** · `source.path === 'plugins/mccp'` ·
`source.ref === 'release'` · **`sha` 키 부재** · **`mccp` 엔트리가 정확히 하나**.
`sha` 항목이 특히 중요하다: 사고 대응 시 `sha`를 일시적으로 박는 것은 허용되지만, 그
"일시"를 지키는 기계가 없으면 조용한 영구 핀이 되어 `ref` 기반 채널이 죽는다.
`url`을 **존재가 아니라 값으로** 재는 것도 같은 무게다 — 이 파일에서 "코드를 어디서
가져오는가"를 정하는 것은 `url` 하나이고, 병합 사고가 그것을 다른 저장소로 바꾸면
나머지 단언은 전부 통과한다.

**이 자리에 있던 약점 문장은 M4가 닫았다.** M3까지 이 절은 탐지기가 사이클마다 실행돼야만
작동한다는 점을 자기 약점으로 명시하고, 그 CI화를 backlog가 소유할 축으로 남겨 두었다.
상시 도는 검사가 아니라 각 마일스톤의 검증 블록이 부르는 단언이었으므로, 아무도 부르지
않는 사이클에는 아무것도 재지 않았다는 뜻이다. 지금은
[`scripts/release-manifest-guard.js`](../scripts/release-manifest-guard.js)가 그 단언을
소유하고 `.github/workflows/release-manifest-gate.yml`이 **모든 PR에서** 돌린다. 판별력은
`scripts/tests/release-manifest-guard.test.js`가 8축으로 재고, 워크플로가 그 test를 가드보다
**먼저** 돌린다(가드가 초록인데 가드 자체가 고장 나 있으면 초록의 의미가 없다).

그 워크플로에 `paths` 필터가 **없는 것은 누락이 아니라 이 절의 요구다.** 좁히면 `sha`
핀을 박은 그 PR에서 한 번 붉고 다음 PR부터는 워크플로가 통째로 건너뛰어져, "red가 곧
타이머"라는 설계가 죽고 조용한 영구 핀이 정확히 이 가드가 막으려던 형태로 되살아난다.
비용(모든 PR에 job 하나)은 의도된 것이며 matrix는 단일 OS다 — 이 가드는 JSON 값만
대조하고 경로·개행·권한을 만지지 않는다.

**여전히 재지 않는 것은 custody다.** 좌표가 옳은지는 보지만, 그 좌표가 직접 push로부터
보호되는지나 이 검사가 `main`의 required status check로 지정됐는지는 보지 않는다.
형태이지 보관이 아니다 — 7절이 그 잔여를 소유한다.
...
- **브랜치 보호는 `release`에도 `main`에도 없다.** 2026-09-04 재측정(M4, 읽기 전용):
  `repos/idenn207/mccp/branches/release/protection`과 `.../main/protection`이 **둘 다**
  404다. M3은 `release`만 쟀기 때문에 "release만 무보호"로 읽힐 여지가 있었는데, 실제로는
  이 저장소에 보호 규칙 자체가 없다. 켜지 않는 이유는 (i) 오늘의 위협 모델이 계정 침해가
  아니라 운영자 오조작이고 그것은 4.1절의 lease 규칙이 다루며 (ii) 보호와 롤백의 강제
  이동이 어떻게 상호작용하는지가 측정된 바 없어, 검증 없이 켜면 사고 대응 경로를 사고 중에
  처음 발견하게 되기 때문이다. 순서 규칙은 그대로다 — **롤백 왕복을 먼저 측정하고 그 다음에
  보호를 켠다.** 측정과 설정은 여전히 backlog 축이며 M4는 그것을 닫지 않았다.
- **6절의 좌표 가드는 형태를 재지 custody를 재지 않는다.** `release-manifest-gate`가 초록인
  것은 좌표가 옳다는 뜻이지, 그 좌표가 직접 push로부터 보호된다는 뜻이 아니다. 이 검사가
  `main`의 required status check로 지정됐는지도 미측정이다 — 지정되지 않은 검사는 붉어도
  머지를 막지 않는다.
