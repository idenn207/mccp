...
|---|---|---|---|
| codex-companion.mjs 인터페이스가 1.0.4 specific. 향후 codex plugin 업데이트시 깨질 수 있음 | M | H (모든 게이트 fallback 회귀) | **Mitigated**: codex-invoke.js의 `verifyCompanionInterface()`가 plugin.json semver를 known-compatible list와 비교 → mismatch 시 `blocking=true` fail-closed. 사용자가 의식적으로 `MCCP_ALLOW_CODEX_UNAVAILABLE=1`로 advisory 전환. |<!--mccp:resolved reason="plan이 .claude/PRPs/plans/completed/ 로 아카이브됨 = gate chain 통과 후 ship 완료, 완화책이 구현되고 테스트로 가드됨" at="2026-06-24T16:29:04.758Z"-->
| auto-chain 도중 사용자가 KILL 못 하면 위험 commit 발생 가능 | M | M | **Mitigated** (Codex R1 #3): `shouldAbort()` 8 trigger 모두 검사 — `MCCP_AUTO_CHAIN_DISABLE=1` env kill switch, non-converged Codex, HIGH/CRITICAL questions, criticalCategory, missing/stale receipt, nonzero step exit, cost hard ceiling, STATE.md `chain_aborted`. |<!--mccp:resolved reason="plan이 .claude/PRPs/plans/completed/ 로 아카이브됨 = gate chain 통과 후 ship 완료, 완화책이 구현되고 테스트로 가드됨" at="2026-06-24T16:29:04.758Z"-->
| receipt soft-mode가 hard에 의존하던 downstream 로직을 silently bypass | H | H (Codex R1 #2: hollow gate) | **Mitigated**: 기본값 `hard`로 변경. `soft`는 opt-in이며, soft에서도 누락 receipt는 즉시 `decision="skipped-soft", blocking=true` placeholder를 write — downstream validator는 non-approving으로 처리. `off`는 큰 stderr 경고. |<!--mccp:resolved reason="plan이 .claude/PRPs/plans/completed/ 로 아카이브됨 = gate chain 통과 후 ship 완료, 완화책이 구현되고 테스트로 가드됨" at="2026-06-24T16:29:04.758Z"-->
| Codex 경로 fix가 registry missing/stale 시 fail-open (Codex R1 #1) | M | H | **Mitigated**: codex-invoke.js의 `resolveCodexInstallPath()`가 registry-missing/registry-malformed/install-path-stale 모두 throw → caller가 `classification!=='ok'`로 받아 `blocking=true`. CLI exit code 12. |<!--mccp:resolved reason="plan이 .claude/PRPs/plans/completed/ 로 아카이브됨 = gate chain 통과 후 ship 완료, 완화책이 구현되고 테스트로 가드됨" at="2026-06-24T16:29:04.758Z"-->
| Cost hard ceiling이 STATE.md만으로는 advisory (Codex R1 #4: race) | H | H | **Mitigated**: auto-chain.js가 mutating step **직전**에 `~/.claude/plugins/data/mccp/cost-current.json`을 직접 read. `hard_ceiling_reached===true`면 abort, 파일 unreadable이면 conservative abort. STATE.md는 보조 channel (reporting + manual override). |<!--mccp:resolved reason="plan이 .claude/PRPs/plans/completed/ 로 아카이브됨 = gate chain 통과 후 ship 완료, 완화책이 구현되고 테스트로 가드됨" at="2026-06-24T16:29:04.758Z"-->
| main 직접 push에 사용자 패턴(PR 검토) 충돌 | H | L (사용자가 명시적으로 main push 선택) | 사용자 선택을 따름. 다음 cycle부터는 PR 패턴 복귀 권장 — plan 끝에 followup 기록. |<!--mccp:resolved reason="plan이 .claude/PRPs/plans/completed/ 로 아카이브됨 = gate chain 통과 후 ship 완료, 완화책이 구현되고 테스트로 가드됨" at="2026-06-24T16:29:04.758Z"-->
| Q10 변경(064c227 CLAUDE.md 등)이 marketplace에 미반영 | L | M | 본 plan 종료 후 사용자에게 다음 세션 시작 시 `/plugin update` 안내. |<!--mccp:resolved reason="plan이 .claude/PRPs/plans/completed/ 로 아카이브됨 = gate chain 통과 후 ship 완료, 완화책이 구현되고 테스트로 가드됨" at="2026-06-24T16:29:04.758Z"-->
...
본 섹션은 `/mccp:prp-implement` Phase 2.5 (Implement-Codex Gate)에서 작성되었습니다. Plan 본문의 `## Codex Adversarial Review`와 분리해 implement-time decisions만 review 대상으로 둡니다. 호출: 동일 Bash 직접 방식 (`node codex-companion.mjs adversarial-review --wait --scope working-tree`).

### Implement-time decisions challenged

1. **R3#2 carry-over** (plan-level DIVERGENT_UNRESOLVED) — terminal `pr` + auto-chain `pr` step에서 `MCCP_ALLOW_CODEX_UNAVAILABLE=1` advisory mode 처리. plan working decision은 hard refusal. force flag (`MCCP_FORCE_PR_WITHOUT_CODEX=1`) 도입 여부 결정 필요.
2. `codex-invoke.js` error 구조 (single ErrorClass + reason field vs. subclass 다발)
3. `cost-current.json` atomic write 구현 디테일 — lockfile 필요 여부
4. CLI exit-code semantics — parse-error / stdout-empty / spawn-enoent 처리
5. `auto-chain.js`의 cost read 위치 (매 step별 vs. cache)

### Round 1 (Codex)

- 호출: `node codex-companion.mjs adversarial-review --wait --scope working-tree "<focus>"`
- session: thread `019e8f15-f7f4-7e22-8670-b618e87c178d` ([.git/mccp/tmp/implement-codex-r1.txt](.git/mccp/tmp/implement-codex-r1.txt))
- verdict: needs-attention. 3 findings (2 high + 1 medium).

수용 — implement 적용 commitment:

- **R1#1 [high]** — pr.md / auto-chain.js의 `pr` step은 `MCCP_ALLOW_CODEX_UNAVAILABLE=1`을 explicit reject + stderr 경고 + abort. **force flag 도입 안 함** (본 cycle). 향후 force flag가 필요해지면 별도 cycle에서 audited/non-approving/reason-required 조건으로 도입. Task 3 wrapper에 pr-specific branch 추가 + `pr.md` 본문에 거부 분기 + acceptance test (`pr.md`에서 advisory env 거부 검증).
- **R1#2 [high]** — `cost-current.json` 쓰기는 lockfile + monotonic write로 강화. 구현: `.claude/plugins/data/mccp/cost-current.lock` 파일을 `wx` flag로 create-or-fail 획득(짧은 spin retry 5회까지). monotonic 룰 — 새 write 시 기존 파일 read 후 `hard_ceiling_reached = prev.hard_ceiling_reached || new.hard_ceiling_reached` (sticky OR), `cost_usd = max(prev.cost_usd, new.cost_usd)`, `last_write_ts = max(...)`, ts가 prev보다 작으면 skip. concurrent writer race test fixture 추가 — true racing false → 최종 file에 hard_ceiling_reached=true 유지.
- **R1#3 [medium]** — `codex-invoke.js`는 `process.execPath`로 companion 호출. spawn ENOENT / parse-error (stdout 형식 위반) / stdout-empty 모두 blocking classification + 항상 valid JSON 출력 + exit 12. fixture 3종 추가 (spawn-enoent, stdout-empty, parse-error).

거부: 없음 (R1 findings 모두 수용).

Open Questions (R1 명시):

- Single ErrorClass vs. subclasses 선택은 boundary에서 한 곳에서 catch + classification/reason contract로 serialize되면 sufficient — single ErrorClass with `reason` enum 채택 (단순성). Codex가 blocker 아님으로 판정.
- auto-chain cost read 위치 — per mutating step (plan body 유지) — Codex sufficient 판정.

### Round 2 (Codex)

- 호출 시 commitment 위 R1 수용 사항 반영하여 challenge.
- session: thread `019e8f1e-...` (background buh22v8do, [.git/mccp/tmp/implement-codex-r2.txt](.git/mccp/tmp/implement-codex-r2.txt))
- verdict: needs-attention. 2 findings (2 high). 둘 다 R1 commitment에 대한 hardening.

수용 — implement 적용 commitment 보강:

- **R2#1 [high]** — cost-state 경로 분기 + ts skip이 sticky safety를 깨뜨림.
  - **Canonical path 통일**: cost-state는 single canonical home path으로만 access. `getCostStateDir() = path.join(os.homedir(), '.claude', 'plugins', 'data', 'mccp')`. lock + json 모두 이 dir 내부. cwd-relative path 사용 금지. helper export로 ecc-context-monitor / auto-chain / state-writer 모두 동일 경로.
  - **Sticky-safety unconditional merge**: 새 write 전 prev read 시 `hard_ceiling_reached = prev.hard_ceiling_reached || new.hard_ceiling_reached`, `cost_usd = max(prev.cost_usd, new.cost_usd)` — 두 필드 모두 **ts와 무관하게 항상 merge**. ts 비교는 `last_write_ts = max(prev.last_write_ts, new.last_write_ts)`에만 적용. 즉 stale event도 hard_ceiling이 true면 sticky하게 유지.
  - **Test 추가**: (a) cross-CWD writers — 두 다른 cwd에서 호출해도 같은 canonical path을 쓰는지, (b) older-true-vs-newer-false race — old(hard_ceiling=true) write가 new(hard_ceiling=false) 뒤에 도착해도 최종 file은 `hard_ceiling_reached=true`.

- **R2#2 [high]** — `pr.md`의 advisory-env rejection이 too late (Phase 1 `gh pr list` 후).
  - **Phase 0 preflight 신설**: `pr.md` 본문 최상단 (Phase 1 discovery 전, 어떤 `gh` 호출 전)에 `MCCP_ALLOW_CODEX_UNAVAILABLE=1` 체크 + 즉시 stderr 경고 + exit 1. **gh 호출 없이 exit + no receipt write**.
  - `auto-chain.js`의 `pr` step도 동일 — chain start 직후, pr.md 호출 전 preflight.
  - **Test 추가**: `gh` shim (fail-on-invocation)이 set된 상태에서 `MCCP_ALLOW_CODEX_UNAVAILABLE=1` 환경으로 `pr.md`를 호출 → gh 호출 0건 + exit 1 + receipt store 변화 없음. 추가로 auto-chain `pr` step도 동일 검증.

거부: 없음 (R2 findings 모두 수용).

### Round 3 (Codex)

- 호출 시 R2 commitment 적용 verify.
- session: thread `019e8f20-0fc9-7140-be4b-4e1bff4b3261` ([.git/mccp/tmp/implement-codex-r3.txt](.git/mccp/tmp/implement-codex-r3.txt))
- verdict: **needs-attention (DIVERGENT_UNRESOLVED at cap)**. 3 findings, 모두 "commitment를 아직 implement하지 않았다"는 process artifact (새로운 architectural finding 아님).
  - R3#1 — prp-implement.md:123 still calls `Skill(codex:adversarial-review)`. Task 1+3 implement 필요.
  - R3#2 — pr.md Phase 0 preflight 미작성. Task 3 + new preflight 필요.
  - R3#3 — ecc-context-monitor cost-current 미작성, auto-chain.js 부재. Task 7 + Task 5 필요.
- Auto-CRITICAL 카탈로그 매칭: §0 (secret/data loss/auth bypass/external destination/crypto) 어느 항목과도 매칭 없음 → MCCP-GATE-STOP 미발동.
- 합치 결론: 본 R3 verdict는 "implement after approve" 순서 process feedback이므로, R1+R2 commitment 그대로 Phase 3에서 implement 진행. Post-implement Codex re-review는 Phase 7 auto-chain `pr` step의 PR-Codex gate에서 자연스럽게 일어남.

### Open Questions (carry-over to Phase 3)

- R3#2 (plan-level) — terminal advisory policy: **본 cycle 결정 = hard refusal**. force flag 미도입. 향후 audited/non-approving force flag 검토는 v0.2.3+ followup.

### Phase 3 진입 조건

- Gate 결과: R1 needs-attention(수용) → R2 needs-attention(수용) → R3 needs-attention(process artifact, DIVERGENT_UNRESOLVED at cap). Auto-CRITICAL 미발동.
- Implement-Codex receipt는 `decision="DIVERGENT_UNRESOLVED_PROCESS"` 형태로 작성 — non-approving receipt이지만 R1/R2/R3 records를 chain-of-custody로 보존. Downstream PR-Codex gate가 final approver 역할.
