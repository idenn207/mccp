...
`my-claude-code-plugin` (mccp)의 게이트·hook·자동화 layer가 인식하는 환경변수 레퍼런스. 상태(✅ live / 📖 LLM-observed / ♻ ECC fork 이관)와 default, 의미, 설정 위치를 정리합니다. `📖 LLM-observed`는 문서·prompt에 등장하지만 hook/script가 mechanical하게 honor하지 않는 변수 — LLM이 자기 환경에서 읽어 동작을 조정합니다 (사용자가 mechanical 강제를 기대하면 fail-open).
...
## 5. mccp Auto-handoff (S10b ✅ live since v0.3.0 · spawn은 v1.1.0 experimental opt-in)

| 변수 | 값 | Default | 설명 |
| --- | --- | --- | --- |
| `MCCP_AUTO_HANDOFF` | `off` \| `notify` \| `spawn` | `notify` | 누적 비용($50 notice / $80 soft / $100 hard) 임계에서 자동 세션 전환 동작. `off`=no-op, `notify`=desktop notification + stdout meta, `spawn`=tmux new-window 또는 Windows Start-Process(race-lock). `spawn`은 v1.1.0부터 `MCCP_AUTO_HANDOFF_EXPERIMENTAL_SPAWN=1` 없이는 `notify`로 강등된다. |
...
> ✅ `/mccp:setup`은 ship됐다. 아래 예시는 그대로 유효하다.
...
CLAUDE.md §4 「운영 토글」이 싣던 **56개 토글의 원문 서술**이다. CLAUDE.md는 세션마다
자동 주입되는 지시문이라 44,462 B(그 파일의 27.6%)를 토글 해설로 쓸 수 없어, 원문을
손실 없이 여기로 옮기고 CLAUDE.md에는 `env · default · 효과` 요약 표만 남겼다.

**여기 있는 것은 배경이다 — 계약이 아니다.** 운영에 필요한 default와 kill-switch는
CLAUDE.md 표에 남아 있다. 이 문서가 답하는 것은 *왜 그 default인가*뿐이다.

각 절은 CLAUDE.md에 있던 줄을 **한 글자도 다듬지 않고** 코드 블록에 그대로 담는다 —
이전이 재작성으로 변질되지 않았음을 줄 단위로 기계 검증할 수 있어야 하기 때문이다.

원본 §4 블록의 그룹 구분 주석 원문:
...
**default**: `observe` · **효과**: stop 직전 자동 게이트 모드 · `off`=끔
...
**default**: `0` · **효과**: stop-loop의 Codex diff review opt-in
...
**default**: `hard` · **효과**: receipt 게이트 강도 · `off`=비활성(디버그)
...
**default**: unset · **효과**: 한 호출만 receipt 게이트 bypass
...
**default**: unset · **효과**: receipt 디버그 출력
...
**default**: unset · **효과**: Codex 실패 시 advisory 진행 · terminal `/mccp:pr`은 거부
...
**default**: unset · **효과**: Codex 호출 영구 skip(spawn 직전 short-circuit)
...
**default**: unset · **효과**: security-reviewer 부재 시 audited override
...
**default**: unset · **효과**: impeccable 부재 시 audited override(strict reason)
...
**default**: unset · **효과**: PR단계 Codex 호출 자체 skip(audited)
...
**default**: unset · **효과**: non-approving ship gate override(audited)
...
**default**: unset · **효과**: dedupe 발화 내부 신호(사용자가 set할 값 아님)
...
**default**: `1` · **효과**: design/a11y scope split · `0`=kill switch
...
**default**: `2` · **효과**: critique retry cap · `0`=R0 1회만
...
**default**: unset · **효과**: design trigger 강제(axis c, strict reason)
...
**default**: unset · **효과**: PR chain-check advisory 우회(audited)
...
**default**: `0` · **효과**: critique 결과 mock(test 전용)
...
**default**: `enforce` · **효과**: produced-diff grounding lint · `off`=끔
...
**default**: `auto` · **효과**: impeccable 명령 라우팅 모드
...
**default**: unset · **효과**: mood 명령 invoke 승격 허용 목록
...
**default**: `1` · **효과**: PR단계 a11y-architect 자동 호출 · `0`=끔
...
**default**: `1` · **효과**: debug ALLOW-path systemMessage · `0`=옛 inline
...
**default**: unset · **효과**: auto-chain kill switch(mechanical)
...
**default**: unset · **효과**: commit까지만 · **LLM-observed**(강제 아님)
...
**default**: `1` · **효과**: work implement 격리 위임 · `0`=인라인
...
**default**: `0` · **효과**: 격리 채널을 Workflow로 · `0`=Task
...
**default**: `on` · **효과**: N-worker 병렬 implement · `off`/`0`=단일
...
**default**: `worktree-merge` · **효과**: 병렬 구조 gate · `disable-parallel`=N=1
...
**default**: `150000` · **효과**: worker당 최소 예상 토큰
...
**default**: empty · **효과**: fleet을 막을 cost tier(명시가 우선)
...
**default**: `enforce` · **효과**: commit 전 통합 diff verify · `off`=끔
...
**default**: `on` · **효과**: plan GROUND 병렬 fan-out · `off`=끔
...
**default**: `150000` · **효과**: 관점당 최소 예상 토큰
...
**default**: empty · **효과**: fan-out을 막을 cost tier(명시가 우선)
...
**default**: on · **효과**: cost-state 부재를 green 가정 · `0`=fail-closed
...
**default**: `24` · **효과**: 세션 누적 worker 절대 상한(원자 예약)
...
**default**: `600000` · **효과**: pending 예약 lease · `0`은 kill switch 아님
...
**default**: `500` · **효과**: 대체 bomb detector 임계(항상 유효)
...
**default**: `off` · **효과**: operational USD 차단 복원(back-compat)
...
**default**: `off` · **효과**: USD 게이트를 context-overflow 축으로 대체
...
**default**: `35` · **효과**: context 잔여% warning 임계
...
**default**: `25` · **효과**: context 잔여% critical 임계
...
**default**: `0` · **효과**: tool_count warning 임계 · `0`=비활성
...
**default**: `0` · **효과**: tool_count critical 임계 · `0`=비활성
...
**default**: `6` · **효과**: cost-state decay 시간 · `0`=decay 끔
...
**default**: `enforce` · **효과**: evidence write lock · `off`=guard 끔
...
**default**: `notify` · **효과**: handoff 신호 동작 · `off`=끔
...
**default**: unset · **효과**: 실제 세션 spawn opt-in
...
**default**: `50,80,100` · **효과**: notice/warning/critical USD 임계
...
**default**: `500` · **효과**: dispatch-watcher polling 간격
...
**default**: `0` · **효과**: =1이면 worker attribution 3필드 강제
...
**default**: `auto` · **효과**: receipt briefing 호출 · `off`=끔
...
**default**: `notice,warning,critical` · **효과**: briefing 자동 disable tier
...
**default**: `5000` · **효과**: STATUS 렌더 debounce
...
**default**: `90000` · **효과**: 렌더 lock lease
...
정의가 사라졌지만 본문이 은퇴 사실을 서술하는 식별자다. CLAUDE.md 요약 표에는
넣지 않는다 — 존재하지 않는 토글의 default를 적으면 운영자를 오도한다.

#### MCCP_ORCHESTRATION_DEBT_DECAY_HOURS

v1.22.3 M3 follow-up 7라운드에서 도입됐다가 같은 cycle의 PR-Codex 5라운드에서
**기각·삭제**됐다. debt 마커가 컨트롤러 death 이후에도 존재한다는 것 자체가 그
agent들이 실제로 떴다는 증거이므로, 시간축 aging-out은 `readCounter`가 실 launch를
차감하게 만드는 under-count(cap이 절대 틀리면 안 되는 over-permissive 방향)가 된다.
따라서 debt pin은 **영구**이며 이 토글은 존재하지 않는다. 자세한 내력은
CLAUDE.md 요약 표의 `MCCP_ORCHESTRATION_RESERVATION_LEASE_MS` 항목과 위
[전수 레퍼런스](#전수-레퍼런스)의 동명 절을 보라.
