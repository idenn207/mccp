...
- **패널보다 앞선 앵커는 짝이 아니다.** 짝은 패널 시각 **이후**의 최초 후보로만 맺는다.
  이전에는 이후 후보가 없으면 가장 최근의 *이전* 후보로 폴백해, 같은 plan을 재리뷰하면
  그 패널이 **직전 lifecycle의 ship**과 짝지어졌다 — 음수 span이 나오면서도 그 짝이
  `by_anchor`·백분위·커버리지에 그대로 남았다. 관측되지 않은 ship을 "매치"로 세는 것이라
  UI6("과거 시각을 추정해 미짝을 메우지 않는다")의 위반이었다(PR-Codex R2 F1).
  이제 그런 후보는 짝을 만들지 않고 `pre_panel_anchors[]`에 **lag과 함께 보고**되며,
  해당 레코드는 증인 규칙으로 분류된다. 조용히 버리지 않는 것이 DD6의 취지였고 그것은
  유지된다 — 바뀐 것은 그 사실을 *짝으로 세지 않는다*는 점이다. 오늘 **0건**이다.
- 그 결과 `negative_spans[]`는 **구조적으로 도달 불가**다. 키는 스키마 안정성을 위해
  남지만 살아있는 신호가 아니라 잔존 가드이며, 그 도달 불가 자체를 회귀 test가 고정한다.
  비어 있는 것을 "음수 span이 없었다"는 관측으로 읽지 말 것 — 만들 수 없을 뿐이다.
