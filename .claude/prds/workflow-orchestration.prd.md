...
| 4 | 병렬 활성화 (worktree-merge live) | worktree→parent merge의 live harness 상관(Workflow worktree↔dispatchId) 입증 → `merge_strategy`를 `worktree-merge`로 승격해 N-worker 병렬 실행 해금. M2b 병렬 스캐폴드 + M3 worktree-merge substrate는 완비, cost hard-ceiling으로 미실측된 live 상관만 잔여 | pending | — |
