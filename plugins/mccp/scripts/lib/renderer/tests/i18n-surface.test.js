...
test('html — footer version v1.19.2 (footer element anchored)', () => {
  const r = renderWithStubs(makeFullModel(Date.now()));
  ...
  assert.match(r.html, /<footer[^>]*>v1\.19\.2 ·/);
...
test('markdown — footer with v1.19.2 version', () => {
  const r = renderWithStubs(makeFullModel(Date.now()));
  assert.match(r.md, /v1\.19\.2/);
});
