...
test('html — footer version v1.22.7 (footer element anchored)', () => {
  const r = renderWithStubs(makeFullModel(Date.now()));
  ...
  assert.match(r.html, /<footer[^>]*>v1\.22\.7 ·/);
...
test('markdown — footer with v1.22.7 version', () => {
  const r = renderWithStubs(makeFullModel(Date.now()));
  assert.match(r.md, /v1\.22\.7/);
});
