...
claimedComputable에서 **제거**(`[A1, B3]`만 잔류), `computeB2/A4/A2`는 **무조건 forward-only**,
...
## 설계 근거 — 왜 셋 다 C1-패턴이고 A1/B3만 claimed인가 (Plan-Codex R1/R2/R3 흡수)
...
- **claimed-computable 자격 = "computed를 정직하게 낼 수 있는가"** — 여기엔 **computed-ZERO**도 포함된다.
  - **A1 (유지)**: `completions_producer_present`가 `task_completed` **KIND 이벤트 존재**로 live flip(session-activity.js:76-78).
    producer가 배선되면 완료가 0이어도 `computed 0/startups`를 정직히 낸다 — 진짜 claimed-computable.
...
| claimedComputable 정책 | `plugins/mccp/scripts/derive/cli.js:210-220` | producer 신호로 computed(zero 포함) 가능한 metric만 유지(A1/B3). 오염/부재는 제거 |
...
| `plugins/mccp/scripts/derive/cli.js` | UPDATE | `claimedComputable = [A1, B3]` (B2·A2·A4 제거). 사유 주석(PF2·R3-F0) |
| `plugins/mccp/scripts/hooks/session-end.js` | UPDATE | (PF3) `context_remaining_pct: null` emit — 검증 불가 latent-wins 값 기록 중단. 사유 주석 |
| `plugins/mccp/scripts/lib/msw-metrics/fixture.js` | UPDATE | 주석만 갱신(claimed=`[A1,B3]`, A2/A4/B2는 C1-패턴 forward-only). **flag 주입 없음**. 기존 데이터는 무해하게 잔류 |
...
| `plugins/mccp/scripts/lib/tests/msw-metrics-acceptance.test.js` | UPDATE | claimedComputable=`[A1,B3]` 반영, A2/A4/B2 non-claimed·forward-only 확인 |
...
### Task 2: claimedComputable = [A1, B3] (PF2·R3-F0)
- **Action**: `derive/cli.js:214-220`에서 A2·A4·B2 제거 → `[A1_WORK_COMPLETION_RATE, B3_TOGGLE_AXES]`. 주석: A2/A4는 오염 source, B2는 독립 producer-presence 신호 부재라 computed-zero 불가 → C1과 함께 제외. A1은 task_completed KIND로 computed-zero 가능해 유지.
...
- **Action**: `fixture.js` 상단 주석을 claimed=`[A1,B3]`로 갱신 + "A2/A4/B2는 C1-패턴 forward-only, fixture 미주입(masquerade 회피)". **데이터 구조 자체는 flag 추가 없음**(기존 collision/handoff/context 데이터는 무해 잔류; 필요 시 정리는 선택).
- **Validate**: metrics-assert --fixtures exit 0 (A1/B3만 compute 요구)
...
- **Action**: msw-metrics.test.js / msw-derive-sources.test.js / acceptance —
  ...
  (e) claimedComputable=`[A1,B3]`; A2/A4/B2 non-claimed 확인,
  (f) A1/B3는 fixture에서 여전히 `computed`.
...
node plugins/mccp/scripts/derive/cli.js metrics-assert --fixtures --json   # exit 0 (claimed=[A1,B3])
...
## Acceptance
- [ ] Task 1-5 완료
- [ ] 6개 validation pass (metrics-assert --fixtures exit 0, claimed=[A1,B3])
- [ ] 실 derive·fixture 양쪽: B2/A4/A2=`forward-only`; A1/B3=`computed`
...
- YAGNI Triage:
  ...
  | PF2 claimedComputable가 production forward-only 은폐 | R1 | HIGH | ACCEPT_NOW | 오염/부재 metric 제거 → `[A1,B3]` |
