...
- [x] `stale-audit/enumerate.js:61`(`if (r.resolved) return;`)도 lifecycle-aware로 만들어 audit agent가 완료 plan 위험을 스킵하게 할지 — 일관성 이득 vs 범위 확대. 본 plan은 dashboard render 표면에 국한, enumerate는 backlog 후보. (severity LOW) <!--mccp:resolved reason="M8 결정 = WON'T DO(by design, render 표면 국한). enumerate.js(현재 line 64)는 의도적으로 superset coverage 유지 — enumerate.js:24-31 주석대로 completed/ plan까지 포함해 human-gated /mccp:dashboard-audit 커버리지 보존. lifecycle-aware로 바꾸면(완료 plan 위험 skip) audit 커버리지가 오히려 줄어 truthfulness에 역행. M8 핵심 버그는 *자동* live count inflation(dashboard render)이고 enumerate는 human이 매 항목 gate하므로 무해(plan Risks 표). 일관성 이득 < 범위 확대 + coverage 손실 → backlog entry 불요." at="2026-06-30T11:03:24Z"-->
