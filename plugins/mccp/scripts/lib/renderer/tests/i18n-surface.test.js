...
test('html — footer version v1.21.2 (footer element anchored)', () => {
  const r = renderWithStubs(makeFullModel(Date.now()));
  ...
  assert.match(r.html, /<footer[^>]*>v1\.21\.2 ·/);
...
test('markdown — footer with v1.21.2 version', () => {
  const r = renderWithStubs(makeFullModel(Date.now()));
  assert.match(r.md, /v1\.21\.2/);
});
