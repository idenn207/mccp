...
ok/attribution/redaction 3조건 통과) · 3번(갈래 H 검증이 봉인 존재 하 통과) · 4번
(`codex-reachability`의 분류 수가 CLAUDE.md §3.3의 15종과 1:1 — §6)이다. 즉 네 항목 중
셋이 충족이고 미충족은 2번 하나다. 이것을 green으로 반올림하지 않는다 — 반올림하면 이
milestone이 만들려던 신호가 사라진다.

> **2026-09-03 보정.** 위 문장은 원래 "충족된 것은 1번과 3번이다"로 4번을 빠뜨렸다.
> 4번의 충족은 §2 D행과 §6이 이미 기록하고 있었으므로 누락은 집계 한 줄에만 있었고,
> 누락 방향도 충족을 덜 세는 보수적 쪽이라 반올림은 아니었다. 그럼에도 이 줄만 읽는
> 독자에게는 4번의 상태가 보이지 않으므로 `/mccp:milestone-close` 진행 중 보정했다.
