...
// goal-phase-lock — multi-turn isolation lock lifecycle CLI for /goal
// completion-condition delegation (mccp v1.x.x axis C / M3).
//
// Mirrors plugins/mccp/scripts/lib/ultracode-phase-lock.js (M2 ship, which
// itself mirrors pr-phase-lock v0.2.8 hardened):
//   - F11 token authority split (sha256 hash in lock body; raw token via sidecar)
//   - F3 host-aware tri-state reclaim policy (same-host+pid-alive=NEVER,
//     same-host+pid-dead=reclaim, cross-host=mtime-only, zero-byte/parse-error=mtime-only)
//   - F8 symlink containment (lock parent dir realpath ⊆ <root>/.claude)
//   - F5 lock file mode 0o600 (S1 absorption — also applied to sidecar token file)
//   - H2 sidecar mkdir BEFORE lock open (orphaned-lock prevention)
//
// M3-specific differences vs M2 (ultracode-phase-lock):
//   1. Lock file: <root>/.claude/state/goal-phase.lock
//   2. Sidecar token: <root>/.git/mccp/tmp/goal-token-<run-id>.dat
//   3. STALE_MS_DEFAULT = 90_000 (90s) — multi-turn /goal loop tolerance,
//      heartbeat orchestration ~30s mark. M2's 60s lease too tight for
//      multi-turn evaluator loops where a single user-pause spans 1+ minute.
//   4. milestone_id field (vs M2's task_index) — captured for trace/debug,
//      reclaim policy ignores it.
//   5. owner_session_id field same — F1 caller-identity discriminator surface
//      (Anthropic /goal session_id is session-scoped per docs, so this field
//      is mainly defense-in-depth for other sub-session contexts; lock
//      activation itself is the primary signal — see plan body's `<!-- goal
//      native spec confirmed -->` footer).
//
// Subcommands:
//   enter --run-id <uuid> [--pid <int>] [--milestone-id <id>]
...
const LOCK_FILENAME = 'goal-phase.lock';
const SIDECAR_RELATIVE_TO_GITDIR = path.join('mccp', 'tmp');
const SIDECAR_PREFIX = 'goal-token-';
const SIDECAR_SUFFIX = '.dat';
const STALE_MS_DEFAULT = 90 * 1000;
...
function cmdEnter(args) {
  ...
  if (!runId) {
    process.stderr.write('[goal-phase-lock] enter requires --run-id\n');
    return 2;
  }
  ...
  const lockBody = {
    ...
    milestone_id: args['milestone-id'] || null,
  };
  const body = JSON.stringify(lockBody, null, 2);
  fs.mkdirSync(path.dirname(p), { recursive: true });

  // F8 symlink containment
  ...
  try {
    ...
    process.stderr.write(
      '[goal-phase-lock] symlink-containment: realpathSync failed: ' + err.message + '\n');
    return 18;
  }
  if (!lockParentReal.startsWith(claudeRootReal + path.sep)
      && lockParentReal !== claudeRootReal) {
    process.stderr.write(
      '[goal-phase-lock] symlink-containment: lock parent ' +
      lockParentReal + ' escapes .claude root ' + claudeRootReal + '\n');
    return 18;
  }

  // H2: sidecar dir mkdir BEFORE lock open
  const sp = sidecarPath(args.cwd, runId);
  try {
    fs.mkdirSync(path.dirname(sp), { recursive: true });
  } catch (err) {
    process.stderr.write(
      '[goal-phase-lock] sidecar dir mkdir failed: ' + err.message + '\n');
    return 19;
  }
  ...
  try {
    ...
    if (existing && !existing._parse_error && !existing._zero_byte
      ...
      process.stderr.write(
        '[goal-phase-lock] lock already held by SAME run_id=' + runId +
        ' (pid=' + existing.pid + '); refuse fresh enter\n');
      return 11;
    }
    if (!tryReclaimStaleLock(p)) {
      process.stderr.write(
        '[goal-phase-lock] lock held by another run' +
        (existing && existing.run_id ? ' (run_id=' + existing.run_id + ')' : '') +
        ' and is not stale; refuse fresh enter.\n');
      process.stderr.write('Hint: node goal-phase-lock.js detect-stale\n');
      return 11;
    }
    ...
    try { tryOpen(); }
      ...
      process.stderr.write(
        '[goal-phase-lock] EEXIST after reclaim race: ' + (err2 && err2.message) + '\n');
      return 11;
    }
  }

  // S1 absorption: sidecar token MUST be mode 0o600
  ...
  process.stdout.write(JSON.stringify({
    ...
    milestone_id: lockBody.milestone_id,
    host: lockBody.host,
  }, null, 2) + '\n');
  return 0;
}
...
function cmdExit(args) {
  ...
  const runId = args['run-id'];
  if (!runId) {
    process.stderr.write('[goal-phase-lock] exit requires --run-id\n');
    return 2;
  }
  const lock = readLock(root);
  if (!lock) {
    process.stderr.write('[goal-phase-lock] no active lock to exit\n');
    return 12;
  }
  if (lock._zero_byte) {
    process.stderr.write(
      '[goal-phase-lock] lock is zero-byte; cannot prove ownership — leave for detect-stale\n');
    return 16;
  }
  if (lock._parse_error) {
    process.stderr.write(
      '[goal-phase-lock] lock corrupted (' + lock._parse_error + '); leave for detect-stale\n');
    return 16;
  }
  if (lock.run_id !== runId) {
    process.stderr.write(
      '[goal-phase-lock] run_id mismatch (lock=' + lock.run_id +
  ...
  if (!token) {
    process.stderr.write(
      '[goal-phase-lock] sidecar token missing for run_id=' + runId +
      '; cannot prove ownership — leave for detect-stale\n');
    return 16;
  }
  if (!verifyTokenAgainstLock(token, lock)) {
    process.stderr.write(
      '[goal-phase-lock] ownership-token mismatch (sidecar vs lock); refuse exit\n');
    return 16;
  }
...
function cmdHeartbeat(args) {
  ...
  const runId = args['run-id'];
  if (!runId) {
    process.stderr.write('[goal-phase-lock] heartbeat requires --run-id\n');
    return 2;
  }
  const lock = readLock(root);
  if (!lock) {
    process.stderr.write('[goal-phase-lock] no active lock to heartbeat\n');
    return 12;
  }
  if (lock._zero_byte || lock._parse_error) {
    process.stderr.write('[goal-phase-lock] heartbeat cannot read lock body; skip\n');
    return 15;
  }
  if (lock.run_id !== runId) {
    process.stderr.write(
      '[goal-phase-lock] heartbeat run_id mismatch (lock=' + lock.run_id +
  ...
  if (!token || !verifyTokenAgainstLock(token, lock)) {
    process.stderr.write(
      '[goal-phase-lock] heartbeat token verification failed; no utimes\n');
    return 15;
  }
  const now = new Date();
  try { fs.utimesSync(lockPath(root), now, now); }
  catch (err) {
    process.stderr.write('[goal-phase-lock] heartbeat utimesSync failed: ' + err.message + '\n');
    return 15;
  }
  process.stdout.write(JSON.stringify({
...
function cmdRead(args) {
  ...
  process.stdout.write(JSON.stringify({
    ...
    milestone_id: lock.milestone_id || null,
...
function main(argv) {
  ...
  switch (sub) {
    ...
    default:
      process.stderr.write('goal-phase-lock: unknown subcommand "' + sub + '"\n');
      return 1;
  }
}
