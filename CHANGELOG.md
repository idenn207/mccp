...
> **Note on versioning**: the project ship tag (e.g. `v1.0.0`) and the inner plugin manifest (`plugins/mccp/.claude-plugin/plugin.json` — currently `1.32.9`) are intentionally decoupled. Plugin semver tracks the mccp namespace's internal API surface; project ship tags track W-VERDICT-gated milestones bundled across the repo.

## [1.32.9] — 2026-08-26

> **§3.7**: `1.32.6 → 1.32.9` (**patch** — diverse-agent-review PRD의 단일
> milestone M8이고 PRD 종료 축이 아니다). **번호를 두 칸 상향했다**: origin/main은
> `1.32.6`이지만 미머지 sibling worktree가 `1.32.7`(santa-delta-review)·
> `1.32.8`(env-contract-integrity)·`1.33.0`(multi-session-work-loop-m8)을 이미
> 선언해 1.32.x 최대치 위인 `1.32.9`에 착지한다. 4면(plugin.json · html.js
> page-foot · markdown.js derived 줄 · 이 파일의 `currently` 노트)을 함께 맞췄고
