 ...
 *
 * Enable: Set MCCP_GOVERNANCE_CAPTURE=1
 * Configure session: Set MCCP_SESSION_ID for session correlation
 */
...
function run(rawInput, options = {}) {
  ...
  const sessionId = process.env.MCCP_SESSION_ID || null;
...
if (require.main === module) {
  ...
  process.stdin.on('end', () => {
    const result = run(raw, {
      truncated,
      maxStdin: Number(process.env.MCCP_HOOK_INPUT_MAX_BYTES) || MAX_STDIN,
    });
