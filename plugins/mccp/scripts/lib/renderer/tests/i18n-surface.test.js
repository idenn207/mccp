...
test('html — footer version v1.20.12 (footer element anchored)', () => {
  const r = renderWithStubs(makeFullModel(Date.now()));
  ...
  assert.match(r.html, /<footer[^>]*>v1\.20\.12 ·/);
...
test('markdown — footer with v1.20.12 version', () => {
  const r = renderWithStubs(makeFullModel(Date.now()));
  assert.match(r.md, /v1\.20\.12/);
});
