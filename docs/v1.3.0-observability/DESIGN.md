...
| H3 | NO cards on layout chrome. Sections separated by spacing + 1px border-bottom only. **v1.4.2 carve-out**: `.severity-tag` pill + `.action-prompt code` chip + `.skip-link` + `.copy-btn` + `[role="alert"].s-secret` emergency banner는 interactive affordance / 4-part 컴포넌트 / a11y / alert chrome으로 design intent. | selector-aware: `border-radius:\s*[1-9]` hit 중 carve-out selector 매칭 제외 후 count > 0 = fail | grid-cell + li.open-questions + .risks were all card-shaped (b204510) |
