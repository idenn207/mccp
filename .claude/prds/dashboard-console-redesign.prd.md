...
| 3 | 우측 상세 드로어 + 드로어 derive 추출 | 항목(미해결질문/위험/타임라인/마일스톤) 클릭→native `<dialog>` 우측 overlay 드로어로 상세 표시(Esc·backdrop·키보드). OQ 선택지·위험 시나리오/완화/잔여·receipt 판정/briefing/hash·마일스톤 요약을 derive에서 추출(스키마 확장 허용). 상세 부재 시 graceful degrade. | in-progress | `.claude/plans/dashboard-console-redesign-m3.plan.md` |
