...
| S3.17 | 3.17 impeccable 탐지 계약 (v1.31.1 M1 · v1.31.2 M2 · v1.31.3 M3 — impeccable-detection-contract) | on-demand | - | - | - | (b) 불성립 — 상주 불변식이 모두 기계 판정을 갖는다. **M3가 이전 근거 하나를 정정한다**: 네임스페이스 분리를 `impeccable-resolve.test.js`의 "bare invocation equals the literal name…"이 단언한다고 적혀 있었으나 거짓이었다. 그 test는 `commands/*.md` 전문에서 리터럴을 모으므로, 재배선이 진짜 호출을 전부 걷어내도 impeccable을 부르지 **않는다**고 적은 `plan-prd.md`의 산문 한 줄이 남아 green을 유지했을 것이다. 실제 앵커는 M3가 넣은 `impeccable-guard.test.js`의 짝 단언 (*사본 존재* = *본문의 bare 리터럴 존재*)이고, 재배선 자체는 같은 파일의 "each calling command resolves its call form instead of hardcoding one"이 고정한다. 정리 규칙 셋은 `impeccable-cleanup.test.js`가 규칙별 거부 + 디스크 무변경으로 단언하며, 특히 `shadowed:true`는 `--source project`·`--source user` 양쪽 거부를 함께 고정한다. 모호성 처리는 "ambiguous winner is reported as unknown, not guessed"가 `source`·`path`·`version` 셋의 null을 고정한다. **M2 소비처 배선**: 판정 권한이 `available` 하나라는 것은 `dep-check.test.js`의 `checkImpeccable` 채널 매트릭스 + 양쪽 로드 순서 순환 회귀와 `session-start-dep-check.test.js`의 배너 양방향이 고정하고, `.impeccable/` 극성은 `gitignore-provision.test.js`의 canonical 단언 + 양방향 drift lint가 고정한다. **한계**: 위 test 중 어느 것도 CI에 등재돼 있지 않다(`.github/workflows/`는 셋만 돌린다) — 강제 지점은 각 사이클의 `## Validation`이다. 상세는 `docs/gate-design.md#impeccable-detection`이 소유한다(M2는 `#### setup·경고 정합 (M2)`, M3는 `#### 섀도잉 해소 (M3)`). 이번 주기 미이전(분류만) |
| S3.18 | 3.18 세션 식별은 단일 체인이다 (v1.33.0 — multi-session-work-loop M8 DD1) | resident | - | - | - | (a) 성립 — 이 규칙이 없던 동안 런타임 12곳이 CLI가 설정하지 않는 이름을 단독으로 읽었고, 그 한 줄이 A1·A2·B3 producer 전체를 침묵시켰다(실측: 트리 전체 msw-events 116건 중 kind가 `evidence_guard_active` 한 종류뿐) · (b) **부분 성립** — 부재(`process.env.CLAUDE_SESSION_ID` 0건)는 `session-identity.test.js` (a)가 스캔으로 강제하지만, "파일명 지점은 sanitize를 거친다"는 (b)/(b3)의 **텍스트·호출 검사**이고 데이터 흐름 분석이 아니다. 반환 계약 통일이라는 구조적 보장은 M3 증거 락과 얽혀 별도 축으로 남았다(DD1이 그 잔여를 명시) · (c) 새 소비처를 배선하기 **전에** 알아야 한다 — 사후에 알면 이미 갈라진 체인을 하나 더 만든 뒤다 |
