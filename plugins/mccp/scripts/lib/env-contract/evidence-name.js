...
//           있는데 실제로는 통과하는 이름도 실패다 — 그 **축소** 방향이 아래 :124-127이
//           강제하는 기계다. **증가** 방향은 이 파일이 강제하지 않는다(v1.32.1 M6
//           정정): 목록에 한 줄을 더하는 것을 막는 코드는 없고, `EVIDENCE_DEBT_CEILING`이
//           그것을 *금지*가 아니라 **가시화**한다 — 상수를 함께 올리는 별도 편집이
//           필요하므로 래칫이 느슨해진 사건이 diff에 숫자로 남는다.
//
// **역방향의 범위는 여전히 주장보다 좁고, 그 경계가 v1.32.1 M6에서 옮겨졌다.**
// `scan.js`의 `isExcluded`는 env-contract 구현 디렉토리를 **디렉토리 앵커**로 제외한다
// (registry 선언 행이 자기 자신을 트리거하는 것을 막으려면 필요하다 — M6 이전에는 경로
// substring이라 디렉토리 밖의 같은 이름 조각까지 조용히 면제했다). 그 제외를 그대로 두면
// 이 검사가 자기 구현 디렉토리를 보지 못하므로, `lint.js`의 `L10_REVERSE_SURFACE_POLICY`가
// **역방향에만** `env-contract/value.js`(값 해석 계층 — `IMPECCABLE_*`가 리터럴로 처음
// 나타날 자리)를 더하고 나머지 파일을 이름과 사유로 열거한다. 즉 이 검사가 증명하는 것은
// «어디에도 없다»가 아니라 **«walkSurfaces 범위 + L10 전용 추가분에 없다»** 이다.
// **잔여**: 그 디렉토리의 *다른* 파일이 장래에 실제로 `process.env.IMPECCABLE_*`를 읽으면
// 부재 주장이 다시 넓어진다 — 정책표가 미분류 `.js`를 붉히므로 침묵하지는 않지만,
// «읽기 형태만 따로 스캔하는 2차 검사»는 여전히 존재하지 않는다.
//
