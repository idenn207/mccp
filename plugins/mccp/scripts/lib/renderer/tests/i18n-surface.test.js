...
test('html — footer version v1.22.4 (footer element anchored)', () => {
  const r = renderWithStubs(makeFullModel(Date.now()));
  ...
  assert.match(r.html, /<footer[^>]*>v1\.22\.4 ·/);
...
test('markdown — footer with v1.22.4 version', () => {
  const r = renderWithStubs(makeFullModel(Date.now()));
  assert.match(r.md, /v1\.22\.4/);
});
