...
| --- | --- | --- | --- | --- |
| 1 | PRD-수준 그룹핑 + PE 토대 | self-contained vendored JS 토대(JS off graceful degrade 불변) 위에서 항목을 소속 PRD별 접힘 그룹으로 묶어 표시. JS-0 invariant를 progressive-enhancement로 개정(H-invariant 충돌 정리). | in-progress | [`.claude/plans/dashboard-data-exploration.plan.md`](../plans/dashboard-data-exploration.plan.md) |
| 2 | 필터 + 정렬 | 필터(PRD/plan/진행상태/worktree, 조합 가능) + 정렬(위험도/시간/작업범위/진행순) 컨트롤. JS off 시 전체 표시. | pending | — |
| 3 | 검색 활성화 | 형태만 있던 검색 입력을 실제 클라이언트 필터로 wiring(항목 텍스트 매칭). 단축키 없음. JS off 시 입력 숨김 + 전체 표시. | pending | — |
