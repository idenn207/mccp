...
- [x] ledger 위치 — `.claude/state/STATE.md` 확장 vs 신규 `.claude/state/milestone-ledger.json`. schema + `receipt_hash` carve-out(v1.3.0-m2 briefing 선례) 적용 범위 (plan 결정). <!--mccp:resolved reason="M1 ship 확정 — 두 후보 모두 폐기, .claude/state/completion-ledger/ 디렉토리 per-entry json 채택(distinct filename으로 cross-worktree append merge 충돌 0). schema_version v1 + receipt_hash carve-out 계승 (store.js)" at="2026-06-30T07:10:00Z"-->
- [x] ledger append 안전성 — detached HEAD / dirty working tree 시 skip + `meta.ledger_write_skipped` 신호 + `/mccp:pr` pre-flight가 dirty ledger 감지 시 stage 0 경고/강제 stage. 두 worktree 동시 append 시 merge 정책(append-only array → 양쪽 보존, 충돌 해석은 사용자 pull/rebase) (plan 결정). <!--mccp:resolved reason="M1 ship 확정 — isLedgerAppendSafe가 unborn/detached HEAD + allowlist 적용 후 dirty tree를 unsafe로 skip. 동시 append는 array merge가 아니라 per-entry distinct 파일(decision__hash12.json)이라 양쪽 보존 + git merge 충돌 0 (store.js)" at="2026-06-30T07:10:00Z"-->
- [x] 호스트-프로젝트 버전 신호의 canonical 소스 — 최신 plan 버전 / CHANGELOG / git tag / 프로젝트 meta 중 무엇을 우선할지. 프로젝트마다 다를 수 있으므로 폴백 사다리 정의 (plan 결정). <!--mccp:resolved reason="M2 ship 확정 — host-version.js 폴백 사다리: (1)host meta(package.json/pyproject/Cargo) authoritative → (2)CHANGELOG top heading → (3)git describe --tags → (4)최신 plan cycle prefix → (5)정직한 미상. plugin manifest 의도적 비참조, source 라벨 항상 노출" at="2026-06-30T07:10:00Z"-->
- [x] 위험/OQ 은퇴 매칭 키 — ledger 스냅샷과 현재 plan 본문을 텍스트로 매칭할지 안정 ID로 할지 + 부분 해결(일부 row만 close) 처리 + plan 본문 revise 후 재매칭 (plan 결정). <!--mccp:resolved reason="M3 ship 확정 — 텍스트/안정ID 매칭 둘 다 폐기. 비파괴 trailing mccp:resolved 마커가 유일 진실원(라인 단위라 부분 해결=per-row, 마커가 콘텐츠 라인에 동행해 revise 후 재매칭 불요). bare 체크박스는 resolved 미인정 (resolution-marker.js)" at="2026-06-30T07:10:00Z"-->
- [x] 다음 행동 command surface — STATE.md Next Step이 `/mccp:*` command가 아닌 서술형일 때의 폴백(서술 + 추론된 command 후보) (plan 결정). <!--mccp:resolved reason="M2 ship 확정(M7/M8 강화) — next-action.js resolveNextAction: 서술형/arg-less 명령은 non-copyable prose로 강등(executable=false) + gate-frontier/in-progress-plan 추론 fallback이 실행가능 command 공급, source 항상 노출. hollow command(resume/trace 등) freshness-gated 필터" at="2026-06-30T07:10:00Z"-->
...
- [x] (M3-b) OQ 진실성 — `## Open Questions` 헤딩 아래 결정 로그(`(결정)`/`(해소)`/`(defer)` 접두)가 "미해결 질문"으로 오노출됨(관측: 8 active OQ 전부 결정 로그, 진짜 미해결 ≈0). 접근 A(audit로 결정-로그 마커링, marker-only 원칙 유지) vs B(접두 render-side 인식, 원칙 일부 완화) — Codex 검토 후 결정. A를 보수적 default 추천. <!--mccp:resolved reason="M3-b ship 확정 — 보수적 default 접근 A 채택: 결정 로그는 dashboard-audit가 비파괴 resolved 마커를 적용해 미해결서 제외(render-side 접두 인식 없음, marker-only 원칙 보존). 어떤 parser에도 결정-접두 인식 로직 부재로 교차 검증 (resolution-marker.js + m3 plan 209-213)" at="2026-06-30T07:10:00Z"-->
