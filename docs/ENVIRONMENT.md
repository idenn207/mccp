...
MCCP_SANTA_BLIND_LANE=a|b|off            # v1.28.2 santa-evidence-diversity M1. default: a. /mccp:santa-loop의 **증거 레인 배정**을 정한다 — `a`는 Reviewer A가 블라인드(B는 번들), `b`는 그 반대, `off`는 전원 번들(M1 이전 동작). 블라인드 레인은 파일 번들과 사전 요약을 **받지 않고** 저장소 루트 + 대상 경로 포인터 + "주어진 서술을 사실로 취급하지 말 것" 지시만 받는다. 이 축이 존재하는 이유는 리뷰어 컨텍스트가 전부 오케스트레이터 한 곳에서 나오면 인스턴스를 몇을 띄우든 라운드를 몇을 돌든 그 번들 밖의 사실이 구조적으로 발견 불가능하기 때문이다(#125 실측 — 동일모델 4인스턴스 × 12라운드가 못 찾은 결함을 저장소를 자기 루프로 재탐색한 리뷰어가 첫 라운드에 포착). **`both`(전원 블라인드)는 없다** — 그러면 오케스트레이터가 스코프를 정하는 의미가 사라진다. 배정은 `plugins/mccp/scripts/lib/santa/lanes.js#assignLanes`(순수 oracle)가 결정하고 `cli.js lanes`가 프롬프트를 조립하며 `record --lane blind|bundled`가 선언값을 재도출해 대조한다(불일치는 exit 2 `SANTA_LANE_MISMATCH`, 라운드는 열린 채 남아 재기록 가능). 불량값은 loud stderr warn 후 `a`로 fail-open. **비대칭 주의 — `off`가 덜 엄격하다**(M1 이전으로 되돌린다). 그럼에도 default를 발화 쪽에 두는 이유는 `off`가 default면 오타 하나가 kill switch를 켜고 **그 실행이 M1 이전과 똑같아 보이기** 때문이다(MCCP_SANTA_TERMINATOR가 감수한 비대칭과 같은 근거). **강제 등급의 정직한 천장 — `--lane`은 선언이지 관측이 아니다**: 대조가 막는 것은 커맨드 본문이 oracle을 거치지 않고 레인을 즉흥적으로 정하는 경로이고, 막지 **못하는** 것은 블라인드로 선언된 리뷰어의 프롬프트에 실제로 번들이 없었는지다(셸에서 LLM이 무엇을 받았는지 확인할 방법이 없다). M1은 위조 방지를 주장하지 않으며, 검증은 결과 분포(두 레인이 동시에 놓친 항목 비율)가 맡는다. 완화는 CLI가 프롬프트를 조립해 정직한 경로를 가장 싸게 만드는 것뿐이다. **관측**: 커버리지는 receipt `meta.santa_blind_records`(전 라운드 블라인드 레코드 수) · `meta.santa_blind_rounds`(블라인드 1건 이상인 라운드 수) 두 present-only 정수로 봉인되고, `santa_blind_rounds === santa_rounds`가 "매 실행에서 리뷰어 ≥1명이 번들 미수령"의 기계적 표현이다. `off` 실행도 값 `0`으로 **남는다** — 키 부재는 "레인 축 이전에 쓰였다(모름)"이고 0은 "관측된 0"이라 서로 다른 상태다. **막지 않는 것**: 블라인드 0건 라운드를 M1은 기록만 하고 **차단하지 않는다**(`gate.js`는 P1 소유). M3의 Scope는 Reviewer B 부재 fallback이라 `off`로 레인이 꺼진 경우를 다루지 않으므로 현재 그 강제는 **어느 milestone도 소유하지 않는다** — `.claude/prds/santa-evidence-diversity.prd.md` Open Questions에 등재됨.
MCCP_SANTA_ALWAYS_SCOPE=enforce|off      # v1.29.1 santa-evidence-diversity M2. default: enforce. /mccp:santa-loop의 **상시 스코프 + 정합 rubric** 한 축을 토글한다 — `enforce`면 Step 1이 `cli.js scope-always`로 "현재 decision의 plan + 그 plan이 `**Source PRD**:`로 스스로 선언한 PRD"를 diff 여부와 무관하게 스코프에 합치고, 고정 rubric 1행(`scope-always.js#CONSISTENCY_RUBRIC`)이 그 쌍을 **지금 워킹트리 기준으로** 대조하라고 지시한다. 이 축이 존재하는 이유는 두 문서의 *관계*인 불변식(계획이 단언하는 마일스톤 식별자·수·회부 건수가 PRD와 맞는가)이 PRD가 diff에 없으면 리뷰어가 몇 명이든 구조적으로 검증 불가이기 때문이다(#125 실측). **스코프는 코퍼스가 아니라 폐포다**(M2 DD1) — `.claude/PRPs/**`·`prds/**` 글롭을 문자 그대로 취하면 이 저장소에서 7 MB이고 그것은 "더 많이 보게 했더니 아무것도 못 보게 됐다"가 되므로, 상시 대상은 diff 안의 `*.plan.md` + `.claude/plans/<slug>*.plan.md` + `.claude/PRPs/plans/<slug>*.plan.md`(둘 다 비재귀, `archived/` 제외) + 각 plan이 선언한 PRD로 좁혔다(실측 약 70 KB). `MAX_ALWAYS_PATHS`(40)가 상한이고 **절삭은 조용히 하지 않는다**(`truncated` 수가 출력에 실린다). **`off`는 스코프 추가와 rubric 행을 함께 끈다** — 한 축이므로 스위치도 하나다(DD5): PRD가 스코프에 없는데 "PRD와 대조하라"고만 지시하면 근거를 댈 수 없는 FAIL을 유도하는 소음이 된다. `off`는 plan 파일을 하나도 열지 않는다(kill switch가 비용까지 끈다). 불량값은 loud stderr warn 후 `enforce`로 fail-open. **비대칭 주의 — `off`가 덜 엄격하다**(M2 이전으로 되돌린다). 그럼에도 default를 발화 쪽에 두는 이유는 `MCCP_SANTA_BLIND_LANE`·`MCCP_SANTA_TERMINATOR`와 같다: `off`가 default면 오타 하나가 kill switch를 켜고 **그 실행이 M2 이전과 똑같아 보인다**(상시 스코프가 0건 추가된 라운드는 정상 실행과 구분되지 않는다). **해소 불가 포인터는 드롭하되 조용히 하지 않는다**(DD4) — 선언이 없는 free-form plan, 아카이브로 옮겨진 PRD, 저장소를 벗어나는 선언은 전부 `unresolved[]`에 이유와 함께 남고 Step 1이 stderr로 찍는다. 드롭 자체는 라운드를 막지 않는다(free-form plan은 정상 입력). 실패는 "도출을 시도했는데 못 했다"(exit ≠ 0)뿐이고 그때 Step 1은 **리뷰어를 띄우지 않는다** — diff 스코프만으로 진행하는 fallback을 두면 그 fallback이 곧 기본 경로가 된다(DD3). **관측의 한계 — receipt는 이 축을 봉인하지 않는다**(DD7): 상시 스코프는 라운드 단위 사실인데 `ledger.beginRound`의 라운드 형태는 P0 동결 시그니처라 필드 추가가 P0 재개 사유이고, 리뷰어 envelope로 우회하면 값이 **호출자 선언**이 되는데 `--lane`과 달리 CLI가 Step 1의 판단을 재현할 수 없어 검증 불가능한 필수 플래그가 된다. 그래서 관측 표면은 (a) Step 1의 stderr 출력(`added`·`pairs`·`unresolved`·`truncated`), (b) 블라인드 프롬프트 본문의 상시 항목, (c) 회귀 test 셋뿐이다 — **상시 축이 조용히 0건을 낸 실행은 receipt만 봐서는 M1 시절 실행과 구분되지 않는다**. 이 공백은 `.claude/prds/santa-evidence-diversity.prd.md` Open Questions에 등재됐고, 봉인이 필요하다는 실측이 나오면 P0 재개로 처리한다. **P3 델타 축소에서 상시 대상은 면제다**(DD6, UI8) — 관계 불변식은 계획이 라운드 사이에 수정되므로 매 라운드 재확인 대상이고, 델타가 그것을 잘라내면 이 축은 라운드 1에서만 살아 있는 축이 된다.
MCCP_SANTA_DEGRADE_GATE=enforce|off      # v1.30.0 santa-evidence-diversity M3. default: enforce. /mccp:santa-loop의 **모델 계열 degrade 강등**을 토글한다 — `enforce`면 `seal.deriveVerdict`가 FINAL 라운드 리뷰어들의 `model` 문자열을 `plugins/mccp/scripts/lib/santa/model-diversity.js#familyOf`로 계열 분류해, distinct 계열이 2 미만이거나 `unknown`이 하나라도 섞이면 `converged`를 **`degraded`로 좁힌다**. 이 축이 존재하는 이유는 `codex`도 `gemini`도 없는 머신에서 Reviewer B가 두 번째 Claude Opus로 떨어지는데 그 조합의 NICE가 이종 조합의 NICE와 **어느 표면에서도 구분되지 않았기** 때문이다(라운드 판정도 봉인 verdict도 receipt도 같은 값). **강등은 봉인 층에서만 한다** — 라운드 판정(`gate.decideVerdict`의 `'NICE'|'NAUGHTY'`)은 P0 동결 시그니처이고 `gate.js`는 소유권 표의 P1 행이라, 거기 넣는 경로는 동결 위반과 소유권 침범을 동시에 저지른다. 봉인 verdict는 이미 push를 막는 자리(`santa-loop.md` Step 5.5)이므로 새 차단 배선이 필요 없다. 판정 우선순위는 `divergent` > `degraded` > `converged`이고 degraded는 **converged를 좁히는 것**이지 divergent를 완화하는 것이 아니다(NAUGHTY + 동일 계열은 여전히 divergent — 뒤집으면 비수렴 라운드가 아래 ack로 push 경로를 얻는다). **`familyOf`는 다중매치도 unknown이다**: `claude-gpt-bridge`처럼 두 카탈로그에 동시에 걸리는 문자열에 precedence로 하나를 주면 그 하나가 상대와 달라 곧바로 이종 판정을 사므로, 매치된 계열이 **정확히 1이 아니면** unknown으로 접고 unknown은 degraded다. 미등재 모델의 처방은 게이트 완화가 아니라 카탈로그 1줄 추가다. 불량값은 loud stderr warn 후 `enforce`로 fail-open. **비대칭 주의 — `off`가 덜 엄격하다**(M3 이전으로 되돌린다). 그럼에도 default를 발화 쪽에 두는 이유는 `MCCP_SANTA_BLIND_LANE`·`MCCP_SANTA_ALWAYS_SCOPE`·`MCCP_SANTA_TERMINATOR`와 같다: `off`가 default면 오타 하나가 kill switch를 켜고 **그 실행이 M3 이전과 똑같아 보인다**. **`off`는 verdict 강등만 끄고 관측은 끄지 않는다** — receipt `meta.santa_model_families`(distinct 계열 수) · `meta.santa_model_degraded` · `meta.santa_degrade_reason`은 `off`에서도 그대로 stamp된다. present-only 의미론에서 **키 부재는 "이 축이 없던 시절에 쓰였다(모름)"이고 값은 "관측했다"**라 서로 다른 상태이므로, kill switch가 관측까지 끄면 `off` 실행이 M3 이전 실행과 구분되지 않고 그것이 정확히 이 milestone이 닫으려는 결함의 모양이다. `MCCP_SANTA_ALWAYS_SCOPE`의 "kill switch가 비용까지 끈다"가 여기서는 성립하지 않는다 — 관측이 원장에 이미 있는 문자열에서 파생되므로 **끌 비용 자체가 없다**. **봉인되는 사유는 2값뿐이다**: `same_family` · `unknown_model`. 의도적 비활성(`MCCP_CODEX_DISABLED=1`)과 미가용(PATH 부재)의 구분은 Step 5.5의 **정지 메시지**가 그 자리에서 읽어 설명하고 receipt에는 싣지 않는다 — 봉인 시점에 다시 관측한 값은 리뷰어가 실제로 돈 시점과 어긋날 수 있는데 receipt는 그것을 라운드의 사실처럼 보여준다. **강제 등급의 정직한 천장 — `--model`은 여전히 선언이다**: `record`가 `openai`/`google` 계열 선언에 대해 `codex`/`gemini`의 PATH 존재를 재도출해 없으면 exit 2(`SANTA_MODEL_UNAVAILABLE`, 라운드는 열린 채)로 막지만, 그것이 막는 것은 *설치되지 않은 CLI를 참칭하는* 경로뿐이고 codex가 설치된 상태에서 Claude fallback을 쓰고 `gpt-5.4`라고 적는 것은 막지 못한다(셸에서 어느 모델이 응답했는지 확인할 방법이 없다). `MCCP_SANTA_BLIND_LANE`의 `--lane`과 **같은 천장**이며 검증은 결과 분포에 맡긴다. **막지 않는 것**: 블라인드 레인이 `off`인 실행의 UI3 미충족은 M3 Scope가 아니다(처방이 다르다 — 모델 degrade는 "codex를 설치하라", 레인 degrade는 "`off`를 그만 쓰라"이고 한 verdict에 묶으면 정지 메시지가 어느 처방을 가리키는지 흐려진다). `.claude/prds/santa-evidence-diversity.prd.md` Open Questions에 신규 milestone 후보로 등재됨.
MCCP_SANTA_DEGRADE_ACK="<reason>"        # v1.30.0 santa-evidence-diversity M3 audited override. **default 없음** — 부재가 곧 "승인 없음"이고 그것이 안전한 쪽이다. 봉인된 verdict가 `degraded`일 때 `/mccp:santa-loop` Step 5.5의 push 차단을 여는 유일한 수단이다. 사유는 strict `validateReason`(`plugins/mccp/scripts/receipt/lib/force-override-reason`)에 **위임**한다 — 30자 이상 · 3단어 이상 · 1-token 금칙 · filler 거부이고, `allowCodeVocabulary`는 넘기지 않는다(§3.13.1이 override 표면을 면제 대상에서 명시적으로 제외한다). 규칙을 이 축에서 다시 쓰지 않는 이유는 override 표면마다 기준이 갈리는 것을 막기 위해서다(`gate.js`가 같은 근거로 import한다). **verdict를 재작성하지 않는다** — 봉인은 `degraded` 그대로이고 receipt에는 `review_verdict='divergent'`(어휘 사영) + `meta.santa_model_degraded=true` + `meta.santa_degrade_ack=true` + `meta.santa_degrade_ack_reason`이 함께 남는다. 선례는 `MCCP_FORCE_PR_WITHOUT_CODEX_CONVERGENCE`다. 이것이 없으면 축이 장식이 된다: codex 미설치 머신에서는 **모든** 실행이 degraded라 ack가 `settings.json`에 상주하게 되는데, ack가 verdict를 `converged`로 바꾸면 그 순간부터 degraded 실행 수가 영구히 0이 되어 "degrade 가시화" 지표가 측정 대상을 잃는다. verdict를 그대로 두면 상주 ack 아래에서도 degraded 비율이 계속 세어지고 그 비율이 곧 "이 머신에 codex를 설치할 이유"의 실측이 된다. **판정 주체는 CLI 한 곳이다** — Step 5.5는 `seal`이 반환한 `degradeAck` 불리언을 읽을 뿐 env를 다시 읽지 않는다(두 곳이 각자 해석하면 두 해석이 갈릴 수 있고, 그 형태의 결함은 이 저장소가 여러 번 잡았다). **ack와 사유는 receipt에서 양방향으로 결속된다** — `schema.js`가 둘 중 하나만 있는 receipt를 거부한다(사유 없는 승인 · 적용되지 않은 override의 사유, 두 나쁜 모양을 함께 막는다). 효력을 발휘하지 않은 ack(강등이 일어나지 않은 실행의 상주 env)는 **stamp되지 않는다** — 플래그는 *설정 여부*가 아니라 *효력 발휘 여부*를 뜻한다(§3.13.1 `intent_gate_force_override` 선례).
...
# gate-guard-integrity M2 test env — 비공허성 증명용 주입 통로 (M3/B3에서 등재)
# 둘 다 **test 파일에서만** 소비되며 production 경로에는 분기가 없다. 등재하는
# 이유는 "test 전용은 문서화 대상이 아니다"가 이 저장소의 관례가 아니기 때문이다
