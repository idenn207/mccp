...
const MCCP_IGNORE_BLOCK = [
  ...
  '# The M5 state journal is working-tree only on purpose: STATE.md stays tracked',
  '# and a fresh clone bootstraps the journal from it as a genesis checkpoint, so',
  '# committing a per-session-append JSONL would only add a conflict surface.',
  ...
  '*.delegations.jsonl',
  '',
  '# impeccable tool byproducts. config.json is the SHARED, committed config —',
  "# impeccable's own reference/hooks.md states that per-developer overrides,",
  '# including the install consent decision, live in the gitignored',
  '# config.local.json, so nothing secret belongs in config.json. design.json is',
  '# a GENERATED sidecar the detector rebuilds, so it is a byproduct like the',
  '# rest. The single-file exception is deliberate: it cannot reach',
  '# config.local.json or live/config.json, which stay ignored.',
  '.impeccable/*',
